...
--* Description:             
--*                          Domain specification Root_Object.RPTS
...
package Root_Object.RP.TS is
--
------------------------------------------------------------------------
   --
   --**************  Object Test_Suite Information **************
   --
   -- Object TS is ACTIVE
   --
   --
   ...
   --   Idle.
   --
   -- Object TS has an identifying non-referential attribute Unique_TS_Identifier 
   ...
   type RP_TS_Type is new RP_Type with record
      ...
      R11_A : Root_Object.Object_List.List_Header_Access_Type;
   ...
   procedure Put_Unique_TS_Identifier (
      ...
      Unique_TS_Identifier_Value : in Application_Types.Base_Integer_Type);
   ...
   procedure Put_R10_B  (
      This_Object :    in  Root_Object.Object_Access;
      R10_B_Value : in Root_Object.Object_List.List_Header_Access_Type);

   pragma inline (Get_R10_B);
   pragma inline (Put_R10_B);
   ...
   procedure Put_R11_A  (
      This_Object :    in  Root_Object.Object_Access;
      R11_A_Value : in Root_Object.Object_List.List_Header_Access_Type);
   ...
   -- EVENT DECLARATION
   --
   type RP_TS_Event_Type is new RP_Event_Type with
   record
   ...
   --    
   type RP_TS_Instance_Event_Type is new RP_TS_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type RP_TS_Description_Event is new RP_TS_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type RP_TS1 is new RP_TS_Instance_Event_Type with
   record 
      null;
   end record;

   type RP_TS2 is new RP_TS_Instance_Event_Type with
   record 
      null;
   end record;

   type RP_TS3 is new RP_TS_Instance_Event_Type with
   record 
      null;
   end record;
   ...
   package RP_TS_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   RP_TS_Queue: RP_TS_Event_Queue.FIFO_Type;
