...
--*************************************************************************************
--*                                                                                   *
--* File Name:               r.RP.UNSUPPORTED.ads
...
--* Description:             
--*                          Domain specification Root_Object.RPUNSUPPORTED
...
--  ----------------------------------  
--  package Root_Object.RP.UNSUPPORTED
...
package Root_Object.RP.UNSUPPORTED is
--
------------------------------------------------------------------------
   --
   --**************  Object A_Test_That_Is_Unsupported Information **************
   --
   -- Object UNSUPPORTED is PASSIVE
   --
   --
   --
   -- Object UNSUPPORTED has an identifying non-referential attribute unsupported_id 
   -- and therefore may be uniquely created.
   -- 
   -- Object UNSUPPORTED has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "UNSUPPORTED";

   --
   -------------------------------------------------------------------------------------------------
   --

   type RP_UNSUPPORTED_Type is new RP_Type with record

      --
      -- Non initialised identifying non referential
      unsupported_id : Application_Types.Base_Integer_Type;
      ...
      Unsupported_Counter : Application_Types.Base_Integer_Type := Application_Types.Base_Integer_Type_First;

      --
      R3_A : Root_Object.Object_Access := null;
   ...
   procedure Put_unsupported_id (
      ...
      unsupported_id_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_unsupported_id);
   pragma inline (Put_unsupported_id);
   ...
   procedure Put_Unsupported_Counter (
      This_Object : in Root_Object.Object_Access;
      Unsupported_Counter_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_Unsupported_Counter);
   pragma inline (Put_Unsupported_Counter);
   ...
   procedure Put_R3_A (
      This_Object :   in  Root_Object.Object_Access;
      R3_A_Value : in Root_Object.Object_Access);

   pragma inline (Get_R3_A);
   pragma inline (Put_R3_A);
