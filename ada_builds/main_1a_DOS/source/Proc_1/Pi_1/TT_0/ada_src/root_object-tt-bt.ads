...
--* Description:             
--*                          Domain specification Root_Object.TTBT
...
package Root_Object.TT.BT is
--
------------------------------------------------------------------------
   --
   --**************  Object Basic_Timer Information **************
   --
   -- Object BT is ACTIVE
   --
   -- Object BT participates in a super sub type relationship
   --
   --
   --
   -- Object BT is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   idle,
   --   executing_part_one,
   --   complete,
   ...
   --   deletion.
   --
   -- Object BT is a SUB type of T via Relationship R2
   --
   -- Object BT has an identifying non-referential attribute id 
   -- and therefore may be uniquely created.
   -- 
   -- Object BT has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "BT";

   --
   -------------------------------------------------------------------------------------------------
   --

   type TT_BT_Type is new TT_Type with record
   ...
   -- EVENT DECLARATION
   --
   type TT_BT_Event_Type is new TT_Event_Type with
   record
   ...
   --    
   type TT_BT_Instance_Event_Type is new TT_BT_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type TT_BT_Description_Event is new TT_BT_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type TT_BT1 is new TT_BT_Instance_Event_Type with
   record 
      null;
   end record;

   type TT_BT2 is new TT_BT_Instance_Event_Type with
   record 
      null;
   end record;

   type TT_BT3 is new TT_BT_Instance_Event_Type with
   record 
      null;
   end record;

   type TT_T1 is new TT_BT_Instance_Event_Type with
   record 
      null;
   end record;

   type TT_T2 is new TT_BT_Instance_Event_Type with
   record 
      null;
   end record;
   ...
   package TT_BT_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   TT_BT_Queue: TT_BT_Event_Queue.FIFO_Type;
