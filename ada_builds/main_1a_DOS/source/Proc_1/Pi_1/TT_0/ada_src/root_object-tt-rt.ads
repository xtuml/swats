...
--* Description:             
--*                          Domain specification Root_Object.TTRT
...
package Root_Object.TT.RT is
--
------------------------------------------------------------------------
   --
   --**************  Object Resetting_Timer Information **************
   --
   -- Object RT is ACTIVE
   --
   -- Object RT participates in a super sub type relationship
   --
   --
   --
   -- Object RT is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   idle,
   --   setup,
   --   executing_part_one,
   ...
   --   deletion.
   --
   -- Object RT is a SUB type of T via Relationship R2
   --
   -- Object RT has an identifying non-referential attribute id 
   -- and therefore may be uniquely created.
   -- 
   -- Object RT has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "RT";

   --
   -------------------------------------------------------------------------------------------------
   --

   type TT_RT_Type is new TT_Type with record
   ...
   -- EVENT DECLARATION
   --
   type TT_RT_Event_Type is new TT_Event_Type with
   record
   ...
   --    
   type TT_RT_Instance_Event_Type is new TT_RT_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type TT_RT_Description_Event is new TT_RT_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type TT_RT1 is new TT_RT_Instance_Event_Type with
   record 
      null;
   end record;

   type TT_RT2 is new TT_RT_Instance_Event_Type with
   record 
      null;
   end record;

   type TT_RT3 is new TT_RT_Instance_Event_Type with
   record 
      null;
   end record;

   type TT_RT4 is new TT_RT_Instance_Event_Type with
   record 
      null;
   end record;

   type TT_RT5 is new TT_RT_Instance_Event_Type with
   record 
      null;
   end record;

   type TT_RT6 is new TT_RT_Instance_Event_Type with
   record 
      null;
   end record;

   type TT_T1 is new TT_RT_Instance_Event_Type with
   record 
      null;
   end record;

   type TT_T2 is new TT_RT_Instance_Event_Type with
   record 
      null;
   end record;
   ...
   package TT_RT_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   TT_RT_Queue: TT_RT_Event_Queue.FIFO_Type;
