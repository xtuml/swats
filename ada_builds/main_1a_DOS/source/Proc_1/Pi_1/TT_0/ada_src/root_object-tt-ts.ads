...
--* Description:             
--*                          Domain specification Root_Object.TTTS
...
package Root_Object.TT.TS is
--
------------------------------------------------------------------------
   --
   --**************  Object Test_Scheduler Information **************
   --
   -- Object TS is ACTIVE
   --
   --
   --
   -- Object TS is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   idle,
   --   perform_subsequent_tests,
   --   finished,
   --   setup,
   --   perform_first_test.
   --
   -- Object TS has an identifying non-referential attribute id 
   -- and therefore may be uniquely created.
   -- 
   -- Object TS has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "TS";

   --
   -------------------------------------------------------------------------------------------------
   --

   type TT_TS_Type is new TT_Type with record
      ...
      -- initial_test_id : Application_Types.Base_Integer_Type;
   
      --
      -- Non Identifying referential. Thou Shalt Not Use. 
      -- report_data_id : Application_Types.Base_Integer_Type;
   
      --
      -- Initialised Non identifying non referential
      current_test_number : Application_Types.Base_Integer_Type := Application_Types.Base_Integer_Type_First;
   ...
   procedure Put_current_test_number (
      This_Object : in Root_Object.Object_Access;
      current_test_number_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_current_test_number);
   pragma inline (Put_current_test_number);
   ...
   -- EVENT DECLARATION
   --
   type TT_TS_Event_Type is new TT_Event_Type with
   record
   ...
   --    
   type TT_TS_Instance_Event_Type is new TT_TS_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type TT_TS_Description_Event is new TT_TS_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type TT_TS1 is new TT_TS_Instance_Event_Type with
   record 
      null;
   end record;

   type TT_TS2 is new TT_TS_Instance_Event_Type with
   record 
      test_instance :  Root_Object.Object_Access;
   end record;

   type TT_TS3 is new TT_TS_Instance_Event_Type with
   record 
      test_instance :  Root_Object.Object_Access;
   end record;

   type TT_TS4 is new TT_TS_Instance_Event_Type with
   record 
      null;
   end record;
   ...
   package TT_TS_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   TT_TS_Queue: TT_TS_Event_Queue.FIFO_Type;
