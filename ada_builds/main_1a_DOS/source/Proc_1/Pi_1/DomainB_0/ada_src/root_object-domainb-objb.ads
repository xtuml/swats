...
--* Description:             
--*                          Domain specification Root_Object.DomainBobjB
...
--  ----------------------------------  
--  package Root_Object.DomainB.objB
...
package Root_Object.DomainB.objB is
--
------------------------------------------------------------------------
   --
   ...
   --   Non_Existent,
   --   Idle,
   --   Sync_service_DomainA_to_an_event_DomainB,
   --   Fail.
   --
   -- Object objB has an identifying non-referential attribute idB 
   ...
   type DomainB_objB_Type is new DomainB_Type with record

      --
      -- Non initialised identifying non referential
      idB : Application_Types.Base_Integer_Type;

      --
      -- Initialised Non identifying non referential
      ResultB : Application_Types.Base_Integer_Type := Application_Types.Base_Integer_Type_First;
   ...
   procedure Put_idB (
      ...
      idB_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_idB);
   pragma inline (Put_idB);
   ...
   procedure Put_ResultB (
      This_Object : in Root_Object.Object_Access;
      ResultB_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_ResultB);
   pragma inline (Put_ResultB);
   ...
   -- EVENT DECLARATION
   --
   type DomainB_objB_Event_Type is new DomainB_Event_Type with
   record
   ...
   --    
   type DomainB_objB_Instance_Event_Type is new DomainB_objB_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type DomainB_objB_Description_Event is new DomainB_objB_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type DomainB_objB1 is new DomainB_objB_Instance_Event_Type with
   record 
      paramA : Application_Types.Base_Integer_Type;
      paramB : Application_Types.Base_Integer_Type;
      Test : Application_Types.Base_Integer_Type;
   end record;

   type DomainB_objB2 is new DomainB_objB_Instance_Event_Type with
   record 
      null;
   end record;

   type DomainB_objB3 is new DomainB_objB_Instance_Event_Type with
   record 
      null;
   end record;
   ...
   package DomainB_objB_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   DomainB_objB_Queue: DomainB_objB_Event_Queue.FIFO_Type;
