...
--* Description:             
--*                          Domain specification Root_Object.EventsPriB
...
--  ----------------------------------  
--  package Root_Object.Events.PriB
...
package Root_Object.Events.PriB is
--
------------------------------------------------------------------------
   --
   --**************  Object Priority_B Information **************
   --
   -- Object PriB is ACTIVE
   --
   -- Object PriB participates in a super sub type relationship
   --
   --
   --
   -- Object PriB is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Idle,
   --   Check.
   --
   -- Object PriB is a SUB type of PriA via Relationship R20
   --
   -- Object PriB has an identifying non-referential attribute B_ID 
   -- and therefore may be uniquely created.
   -- 
   -- Object PriB has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "PriB";

   --
   -------------------------------------------------------------------------------------------------
   --

   type Events_PriB_Type is new Events_Type with record

      --
      -- Non initialised identifying non referential
      B_ID : Application_Types.Base_Integer_Type;
      ...
      R20_Super : Root_Object.Object_Access := null;
   ...
   procedure Put_B_ID (
      ...
      B_ID_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_B_ID);
   pragma inline (Put_B_ID);
   ...
   procedure Put_R20_Super (
      This_Object : in  Root_Object.Object_Access;
      R20_Super_Value : in Root_Object.Object_Access);

   pragma inline (Get_R20_Super);
   pragma inline (Put_R20_Super);
   ...
   -- EVENT DECLARATION
   --
   type Events_PriB_Event_Type is new Events_Event_Type with
   record
   ...
   --    
   type Events_PriB_Instance_Event_Type is new Events_PriB_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type Events_PriB_Description_Event is new Events_PriB_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type Events_PriB1 is new Events_PriB_Instance_Event_Type with
   record 
      Test_No : Application_Types.Base_Integer_Type;
   end record;

   type Events_PriB2 is new Events_PriB_Instance_Event_Type with
   record 
      Test_No : Application_Types.Base_Integer_Type;
      Index : Application_Types.Base_Integer_Type;
   end record;

   type Events_PriA1 is new Events_PriB_Instance_Event_Type with
   record 
      Test_No : Application_Types.Base_Integer_Type;
      Index : Application_Types.Base_Integer_Type;
   end record;

   type Events_PriA2 is new Events_PriB_Instance_Event_Type with
   record 
      Test_No : Application_Types.Base_Integer_Type;
   end record;

   type Events_PriA3 is new Events_PriB_Instance_Event_Type with
   record 
      Test_No : Application_Types.Base_Integer_Type;
      Index : Application_Types.Base_Integer_Type;
   end record;
   ...
   package Events_PriB_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   Events_PriB_Queue: Events_PriB_Event_Queue.FIFO_Type;
