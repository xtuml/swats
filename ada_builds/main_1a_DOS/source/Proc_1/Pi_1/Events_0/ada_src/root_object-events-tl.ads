...
--* Description:             
--*                          Domain specification Root_Object.EventsTL
...
package Root_Object.Events.TL is
--
------------------------------------------------------------------------
   --
   --**************  Object Top_Level Information **************
   --
   -- Object TL is ACTIVE
   --
   -- Object TL participates in a super sub type relationship
   --
   --
   --
   -- Object TL is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Check,
   --   Finish.
   --
   -- Object TL is a SUPER type of 
   --    SL_ONE_A,
   --    SL_ONE_B
   --    via Relationship R9
   --
   -- Object TL has an identifying non-referential attribute TL_id 
   -- and therefore may be uniquely created.
   -- 
   -- Object TL has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "TL";

   --
   -------------------------------------------------------------------------------------------------
   --

   type Events_TL_Type is new Events_Type with record
      ...
      R9_Sub : Root_Object.Object_Access := null;
   ...
   procedure Put_TL_id (
      ...
      TL_id_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_TL_id);
   pragma inline (Put_TL_id);
   ...
   procedure Put_R9_Sub (
      This_Object :   in  Root_Object.Object_Access;
      R9_Sub_Value : in Root_Object.Object_Access);

   pragma inline (Get_R9_Sub);
   pragma inline (Put_R9_Sub);
   ...
   -- EVENT DECLARATION
   --
   type Events_TL_Event_Type is new Events_Event_Type with
   record
   ...
   --    
   type Events_TL_Instance_Event_Type is new Events_TL_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type Events_TL_Description_Event is new Events_TL_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type Events_TL1 is new Events_TL_Instance_Event_Type with
   record 
      null;
   end record;

   type Events_TL2 is new Events_TL_Instance_Event_Type with
   record 
   ...
   package Events_TL_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   Events_TL_Queue: Events_TL_Event_Queue.FIFO_Type;
