...
--* Description:             
--*                          Domain specification Root_Object.EventsobjM
...
--  ----------------------------------  
--  package Root_Object.Events.objM
...
package Root_Object.Events.objM is
--
------------------------------------------------------------------------
   --
   --**************  Object Object_M Information **************
   --
   -- Object objM is PASSIVE
   --
   -- Object objM participates in a super sub type relationship
   --
   --
   --
   -- Object objM is a SUPER type of 
   --    objN
   --    via Relationship R2
   --
   -- Object objM has an identifying non-referential attribute Reference_M 
   -- and therefore may be uniquely created.
   -- 
   -- Object objM has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "objM";

   --
   -------------------------------------------------------------------------------------------------
   --

   type Events_objM_Type is new Events_Type with record
      ...
      Result_M : Application_Types.Base_Integer_Type := Application_Types.Base_Integer_Type_First;

      --
      -- Initialised Non identifying non referential
      Current_State :  Object_State_Type :=  Object_State_Type_First;

      R2_Sub : Root_Object.Object_Access := null;
   ...
   procedure Put_Result_M (
      This_Object : in Root_Object.Object_Access;
      Result_M_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_Result_M);
   pragma inline (Put_Result_M);
   ...
   procedure Put_R2_Sub (
      This_Object :   in  Root_Object.Object_Access;
      R2_Sub_Value : in Root_Object.Object_Access);

   pragma inline (Get_R2_Sub);
   pragma inline (Put_R2_Sub);
   ...
   --
   type Events_objM_Event_Type is new Events_Event_Type with
   record
   ...
   --    
   type Events_objM_Instance_Event_Type is new Events_objM_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type Events_objM_Description_Event is new Events_objM_Instance_Event_Type with
   record
      null;
   end record;
   --
