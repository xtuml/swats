...
--* Description:             
--*                          Domain specification Root_Object.EventsCP
...
package Root_Object.Events.CP is
--
------------------------------------------------------------------------
   --
   --**************  Object Check_Parameters Information **************
   --
   -- Object CP is ACTIVE
   --
   --
   --
   -- Object CP is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Idle,
   --   StartA.
   --
   -- Object CP has an identifying non-referential attribute CP_ID 
   -- and therefore may be uniquely created.
   -- 
   -- Object CP has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "CP";

   --
   -------------------------------------------------------------------------------------------------
   --

   type Events_CP_Type is new Events_Type with record

      --
      -- Non initialised identifying non referential
      CP_ID : Application_Types.Base_Integer_Type;

      --
      -- Initialised Non identifying non referential
      Number_Of_Test : Application_Types.Base_Integer_Type := Application_Types.Base_Integer_Type_First;
   ...
   procedure Put_CP_ID (
      ...
      CP_ID_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_CP_ID);
   pragma inline (Put_CP_ID);
   ...
   procedure Put_Number_Of_Test (
      This_Object : in Root_Object.Object_Access;
      Number_Of_Test_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_Number_Of_Test);
   pragma inline (Put_Number_Of_Test);
   ...
   -- EVENT DECLARATION
   --
   type Events_CP_Event_Type is new Events_Event_Type with
   record
   ...
   --    
   type Events_CP_Instance_Event_Type is new Events_CP_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type Events_CP_Description_Event is new Events_CP_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type Events_CP1 is new Events_CP_Instance_Event_Type with
   record 
      Start_Val : Application_Types.Base_Integer_Type;
   end record;

   type Events_CP2 is new Events_CP_Instance_Event_Type with
   record 
      Start_Val : Application_Types.Base_Integer_Type;
      Got_To_End :  Boolean;
   end record;
   ...
   package Events_CP_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   Events_CP_Queue: Events_CP_Event_Queue.FIFO_Type;
