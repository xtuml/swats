...
--* Description:             
--*                          Domain specification Root_Object.EventsobjF
...
--  ----------------------------------  
--  package Root_Object.Events.objF
...
package Root_Object.Events.objF is
--
------------------------------------------------------------------------
   --
   --**************  Object Object_F Information **************
   --
   -- Object objF is ACTIVE
   --
   -- Object objF participates in a super sub type relationship
   --
   --
   --
   -- Object objF is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Idle,
   --   Subtype_object.
   --
   -- Object objF is a SUB type of objD via Relationship R5
   --
   -- Object objF does not have an identifying non-referential attribute and 
   -- therefore may not be uniquely created.
   -- 
   -- Object objF has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "objF";

   --
   -------------------------------------------------------------------------------------------------
   --

   type Events_objF_Type is new Events_Type with record

      --
      -- Non initialised identifying referential
      ReferenceD : Application_Types.Base_Integer_Type;

      --
      -- Initialised Non identifying non referential
      ReferenceF : Application_Types.Base_Integer_Type := Application_Types.Base_Integer_Type_First;
      ...
      --
      -- Initialised Non identifying non referential
      Report_Test_No : Application_Types.Base_Integer_Type := Application_Types.Base_Integer_Type_First;

      --
      -- Initialised Non identifying non referential
      Current_State :  Object_State_Type :=  Object_State_Type_First;

      R5_Super : Root_Object.Object_Access := null;
   ...
   procedure Put_ReferenceD (
      ...
      ReferenceD_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_ReferenceD);
   pragma inline (Put_ReferenceD);
   ...
   procedure Put_ReferenceF (
      This_Object : in Root_Object.Object_Access;
      ReferenceF_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_ReferenceF);
   pragma inline (Put_ReferenceF);
   ...
   procedure Put_Report_Test_No (
      This_Object : in Root_Object.Object_Access;
      Report_Test_No_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_Report_Test_No);
   pragma inline (Put_Report_Test_No);
   ...
   procedure Put_R5_Super (
      This_Object : in  Root_Object.Object_Access;
      R5_Super_Value : in Root_Object.Object_Access);

   pragma inline (Get_R5_Super);
   pragma inline (Put_R5_Super);
   ...
   -- EVENT DECLARATION
   --
   type Events_objF_Event_Type is new Events_Event_Type with
   record
   ...
   --    
   type Events_objF_Instance_Event_Type is new Events_objF_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type Events_objF_Description_Event is new Events_objF_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type Events_objF1 is new Events_objF_Instance_Event_Type with
   record 
      Test_Number : Application_Types.Base_Integer_Type;
   end record;

   type Events_objF2 is new Events_objF_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
      Test_Number : Application_Types.Base_Integer_Type;
   end record;

   type Events_objD1 is new Events_objF_Instance_Event_Type with
   record 
      Test_Number : Application_Types.Base_Integer_Type;
   end record;

   type Events_objD2 is new Events_objF_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
      Test_Number : Application_Types.Base_Integer_Type;
   end record;

   type Events_objD3 is new Events_objF_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
      Test_Number : Application_Types.Base_Integer_Type;
   end record;

   type Events_objD4 is new Events_objF_Instance_Event_Type with
   record 
      null;
   end record;
   ...
   package Events_objF_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   Events_objF_Queue: Events_objF_Event_Queue.FIFO_Type;
