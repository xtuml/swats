...
--* Description:             
--*                          Domain specification Root_Object.EventsobjTP
...
--  ----------------------------------  
--  package Root_Object.Events.objTP
...
package Root_Object.Events.objTP is
--
------------------------------------------------------------------------
   --
   --**************  Object Test_Persistence Information **************
   --
   -- Object objTP is ACTIVE
   --
   --
   --
   -- Object objTP is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Idle,
   --   Stage_A,
   --   Test_Real,
   --   Test_Text.
   --
   -- Object objTP has an identifying non-referential attribute TPID 
   -- and therefore may be uniquely created.
   -- 
   -- Object objTP has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "objTP";

   --
   -------------------------------------------------------------------------------------------------
   --

   type Events_objTP_Type is new Events_Type with record

      --
      -- Non initialised identifying non referential
      TPID : Application_Types.Base_Integer_Type;
   ...
   procedure Put_TPID (
      ...
      TPID_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_TPID);
   pragma inline (Put_TPID);
   ...
   -- EVENT DECLARATION
   --
   type Events_objTP_Event_Type is new Events_Event_Type with
   record
   ...
   --    
   type Events_objTP_Instance_Event_Type is new Events_objTP_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type Events_objTP_Description_Event is new Events_objTP_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type Events_objTP1 is new Events_objTP_Instance_Event_Type with
   record 
      Test_No : Application_Types.Base_Integer_Type;
      A_Val : Application_Types.Base_Integer_Type;
   end record;

   type Events_objTP2 is new Events_objTP_Instance_Event_Type with
   record 
      Test_No : Application_Types.Base_Integer_Type;
   end record;

   type Events_objTP3 is new Events_objTP_Instance_Event_Type with
   record 
      null;
   end record;

   type Events_objTP4 is new Events_objTP_Instance_Event_Type with
   record 
      Test_No : Application_Types.Base_Integer_Type;
      Real_Val : Application_Types.Base_Float_Type;
   end record;

   type Events_objTP5 is new Events_objTP_Instance_Event_Type with
   record 
      Test_No : Application_Types.Base_Integer_Type;
      Pass_Input :  Root_Object.Object_Access;
   end record;

   type Events_objTP6 is new Events_objTP_Instance_Event_Type with
   record 
      Test_No : Application_Types.Base_Integer_Type;
      Text_Input : Application_Types.Base_Text_Type;
   end record;
   ...
   package Events_objTP_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   Events_objTP_Queue: Events_objTP_Event_Queue.FIFO_Type;
