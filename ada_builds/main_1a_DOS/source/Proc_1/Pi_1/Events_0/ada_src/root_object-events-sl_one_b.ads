...
--*************************************************************************************
--*                                                                                   *
--* File Name:               r.Events.SL_ONE_B.ads
...
--* Description:             
--*                          Domain specification Root_Object.EventsSL_ONE_B
...
--  ----------------------------------  
--  package Root_Object.Events.SL_ONE_B
...
package Root_Object.Events.SL_ONE_B is
--
------------------------------------------------------------------------
   --
   --**************  Object Sub_Level_1_B Information **************
   --
   -- Object SL_ONE_B is PASSIVE
   --
   -- Object SL_ONE_B participates in a super sub type relationship
   --
   --
   --
   -- Object SL_ONE_B is a SUPER type of 
   --    SL_TWO_C,
   --    SL_TWO_D
   --    via Relationship R11
   --
   -- Object SL_ONE_B is a SUB type of TL via Relationship R9
   --
   -- Object SL_ONE_B has an identifying non-referential attribute SL1B_id 
   -- and therefore may be uniquely created.
   -- 
   -- Object SL_ONE_B has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "SL_ONE_B";

   --
   -------------------------------------------------------------------------------------------------
   --

   type Events_SL_ONE_B_Type is new Events_Type with record

      --
      -- Non initialised identifying non referential
      SL1B_id : Application_Types.Base_Integer_Type;

      --
      -- Non Identifying referential. Thou Shalt Not Use. 
      -- TL_id : Application_Types.Base_Integer_Type;
   
      --
      -- Initialised Non identifying non referential
      Current_State :  Object_State_Type :=  Object_State_Type_First;

      R11_Sub : Root_Object.Object_Access := null;
   ...
   function Get_SL1B_id (
      This_Object : Root_Object.Object_Access) 
   return Application_Types.Base_Integer_Type;

   procedure Put_SL1B_id (
      This_Object : in Root_Object.Object_Access;
      SL1B_id_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_SL1B_id);
   pragma inline (Put_SL1B_id);
   ...
   function  Get_R11_Sub (
      This_Object : Root_Object.Object_Access) 
   return Root_Object.Object_Access;

   procedure Put_R11_Sub (
      This_Object :   in  Root_Object.Object_Access;
      R11_Sub_Value : in Root_Object.Object_Access);

   pragma inline (Get_R11_Sub);
   pragma inline (Put_R11_Sub);
   ...
   --
   type Events_SL_ONE_B_Event_Type is new Events_Event_Type with
   record
      This_Event_Class : Event_Class;
   end record;

   procedure Event_Action (
      Dispatch_Event : in     Events_SL_ONE_B_Event_Type;
      This_Event     : in out Root_Object.Root_Event_Access_Type);


   --    
   type Events_SL_ONE_B_Instance_Event_Type is new Events_SL_ONE_B_Event_Type with
   record
   ...
   -- 
   procedure Event_Action (
      Dispatch_Event : in     Events_SL_ONE_B_Instance_Event_Type;
      This_Event     : in out Root_Object.Root_Event_Access_Type);



   ------------------------------------------------------------------------
   type Events_SL_ONE_B_Description_Event is new Events_SL_ONE_B_Instance_Event_Type with
   record
      null;
   end record;
   --
   ...
   --
   --
   type Events_SL_ONE_B1 is new Events_SL_ONE_B_Instance_Event_Type with
   record 
      null;
   end record;
   --
   type Events_TL1 is new Events_SL_ONE_B_Instance_Event_Type with
   record 
      null;
   end record;
   --
   type Events_TL2 is new Events_SL_ONE_B_Instance_Event_Type with
   record 
      Test_No: Application_Types.Base_Integer_Type;
   end record;

end Root_Object.Events.SL_ONE_B;
