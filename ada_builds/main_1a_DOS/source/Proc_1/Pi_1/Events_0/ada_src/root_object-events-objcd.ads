...
--* Description:             
--*                          Domain specification Root_Object.EventsobjCD
...
--  ----------------------------------  
--  package Root_Object.Events.objCD
...
package Root_Object.Events.objCD is
--
------------------------------------------------------------------------
   --
   --**************  Object Check_Delete Information **************
   --
   -- Object objCD is ACTIVE
   --
   --
   --
   -- Object objCD is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Idle,
   --   Check.
   --
   -- Object objCD has an identifying non-referential attribute idCD 
   ...
   type Events_objCD_Type is new Events_Type with record

      --
      -- Non initialised identifying non referential
      idCD : Application_Types.Base_Integer_Type;

      --
      -- Initialised Non identifying non referential
      CDTotal : Application_Types.Base_Integer_Type := Application_Types.Base_Integer_Type_First;
   ...
   procedure Put_idCD (
      ...
      idCD_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_idCD);
   pragma inline (Put_idCD);
   ...
   procedure Put_CDTotal (
      This_Object : in Root_Object.Object_Access;
      CDTotal_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_CDTotal);
   pragma inline (Put_CDTotal);
   ...
   -- EVENT DECLARATION
   --
   type Events_objCD_Event_Type is new Events_Event_Type with
   record
   ...
   --    
   type Events_objCD_Instance_Event_Type is new Events_objCD_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type Events_objCD_Description_Event is new Events_objCD_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type Events_objCD1 is new Events_objCD_Instance_Event_Type with
   record 
      Test_No : Application_Types.Base_Integer_Type;
      Deleted :  Boolean;
   end record;

   type Events_objCD2 is new Events_objCD_Instance_Event_Type with
   record 
      Test_No : Application_Types.Base_Integer_Type;
      Deleted :  Boolean;
   end record;
   ...
   package Events_objCD_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   Events_objCD_Queue: Events_objCD_Event_Queue.FIFO_Type;
