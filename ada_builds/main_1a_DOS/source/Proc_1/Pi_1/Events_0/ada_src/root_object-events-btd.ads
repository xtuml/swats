...
--* Description:             
--*                          Domain specification Root_Object.EventsBTD
...
--  ----------------------------------  
--  package Root_Object.Events.BTD
...
package Root_Object.Events.BTD is
--
------------------------------------------------------------------------
   --
   --**************  Object BornToDie Information **************
   --
   -- Object BTD is ACTIVE
   --
   --
   --
   -- Object BTD is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Creation_state,
   --   Termination_state,
   --   Idle,
   ...
   --   Fail.
   --
   -- Object BTD has an identifying non-referential attribute idBTD 
   -- and therefore may be uniquely created.
   -- 
   -- Object BTD has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "BTD";

   --
   -------------------------------------------------------------------------------------------------
   --

   type Events_BTD_Type is new Events_Type with record

      --
      -- Non initialised identifying non referential
      idBTD : Application_Types.Base_Integer_Type;
   ...
   procedure Put_idBTD (
      ...
      idBTD_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_idBTD);
   pragma inline (Put_idBTD);
   ...
   -- EVENT DECLARATION
   --
   type Events_BTD_Event_Type is new Events_Event_Type with
   record
   ...
   --    
   type Events_BTD_Instance_Event_Type is new Events_BTD_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type Events_BTD_Description_Event is new Events_BTD_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type Events_BTD1 is new Events_BTD_Instance_Event_Type with
   record 
      null;
   end record;

   type Events_BTD2 is new Events_BTD_Instance_Event_Type with
   record 
      null;
   end record;

   type Events_BTD3 is new Events_BTD_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
      Test_Number : Application_Types.Base_Integer_Type;
   end record;

   type Events_BTD5 is new Events_BTD_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
      Test_Number : Application_Types.Base_Integer_Type;
   end record;
   ...
   package Events_BTD_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   Events_BTD_Queue: Events_BTD_Event_Queue.FIFO_Type;
