...
--* Description:             
--*                          Domain specification Root_Object.EventsobjQ
...
--  ----------------------------------  
--  package Root_Object.Events.objQ
...
package Root_Object.Events.objQ is
--
------------------------------------------------------------------------
   --
   --**************  Object Object_Q Information **************
   --
   -- Object objQ is ACTIVE
   --
   -- Object objQ participates in a super sub type relationship
   --
   --
   --
   -- Object objQ is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Idle,
   --   Subtype_State.
   --
   -- Object objQ is a SUB type of objP via Relationship R4
   --
   -- Object objQ does not have an identifying non-referential attribute and 
   -- therefore may not be uniquely created.
   -- 
   -- Object objQ has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "objQ";

   --
   -------------------------------------------------------------------------------------------------
   --

   type Events_objQ_Type is new Events_Type with record
      ...
      --
      -- Initialised Non identifying non referential
      Result_Q : Application_Types.Base_Integer_Type := Application_Types.Base_Integer_Type_First;
   ...
   procedure Put_Result_Q (
      This_Object : in Root_Object.Object_Access;
      Result_Q_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_Result_Q);
   pragma inline (Put_Result_Q);
   ...
   -- EVENT DECLARATION
   --
   type Events_objQ_Event_Type is new Events_Event_Type with
   record
   ...
   --    
   type Events_objQ_Instance_Event_Type is new Events_objQ_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type Events_objQ_Description_Event is new Events_objQ_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type Events_objQ1 is new Events_objQ_Instance_Event_Type with
   record 
      null;
   end record;

   type Events_objP1 is new Events_objQ_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;

   type Events_objP2 is new Events_objQ_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;
   ...
   package Events_objQ_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   Events_objQ_Queue: Events_objQ_Event_Queue.FIFO_Type;
