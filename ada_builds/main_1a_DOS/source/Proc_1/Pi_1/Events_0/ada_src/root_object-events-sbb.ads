...
--* Description:             
--*                          Domain specification Root_Object.EventssbB
...
--  ----------------------------------  
--  package Root_Object.Events.sbB
...
package Root_Object.Events.sbB is
--
------------------------------------------------------------------------
   --
   --**************  Object SubB Information **************
   --
   -- Object sbB is ACTIVE
   --
   -- Object sbB participates in a super sub type relationship
   --
   --
   --
   -- Object sbB is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Idle,
   --   Update.
   --
   -- Object sbB is a SUB type of SprA via Relationship R13
   --
   -- Object sbB does not have an identifying non-referential attribute and 
   -- therefore may not be uniquely created.
   -- 
   -- Object sbB has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "sbB";

   --
   -------------------------------------------------------------------------------------------------
   --

   type Events_sbB_Type is new Events_Type with record
   ...
   -- EVENT DECLARATION
   --
   type Events_sbB_Event_Type is new Events_Event_Type with
   record
   ...
   --    
   type Events_sbB_Instance_Event_Type is new Events_sbB_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type Events_sbB_Description_Event is new Events_sbB_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type Events_SprA1 is new Events_sbB_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;

   type Events_SprA2 is new Events_sbB_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;

   type Events_SprA3 is new Events_sbB_Instance_Event_Type with
   record 
      null;
   end record;

   type Events_SprA4 is new Events_sbB_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;

   type Events_SprA5 is new Events_sbB_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;

   type Events_SprA6 is new Events_sbB_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;
   ...
   package Events_sbB_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   Events_sbB_Queue: Events_sbB_Event_Queue.FIFO_Type;
