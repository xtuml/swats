...
--* Description:             
--*                          Domain specification Root_Object.EventsobjU
...
--  ----------------------------------  
--  package Root_Object.Events.objU
...
package Root_Object.Events.objU is
--
------------------------------------------------------------------------
   --
   --**************  Object Object_U Information **************
   --
   -- Object objU is ACTIVE
   --
   -- Object objU participates in a super sub type relationship
   --
   --
   --
   -- Object objU is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Idle,
   --   Subtype_State.
   --
   -- Object objU is a SUB type of objT via Relationship R8
   --
   -- Object objU does not have an identifying non-referential attribute and 
   -- therefore may not be uniquely created.
   -- 
   -- Object objU has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "objU";

   --
   -------------------------------------------------------------------------------------------------
   --

   type Events_objU_Type is new Events_Type with record

      --
      -- Non initialised identifying referential
      Reference_T : Application_Types.Base_Integer_Type;

      --
      -- Initialised Non identifying non referential
      Result_U : Application_Types.Base_Integer_Type := Application_Types.Base_Integer_Type_First;
      ...
      R8_Super : Root_Object.Object_Access := null;
   ...
   procedure Put_Reference_T (
      ...
      Reference_T_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_Reference_T);
   pragma inline (Put_Reference_T);
   ...
   procedure Put_Result_U (
      This_Object : in Root_Object.Object_Access;
      Result_U_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_Result_U);
   pragma inline (Put_Result_U);
   ...
   procedure Put_R8_Super (
      This_Object : in  Root_Object.Object_Access;
      R8_Super_Value : in Root_Object.Object_Access);

   pragma inline (Get_R8_Super);
   pragma inline (Put_R8_Super);
   ...
   -- EVENT DECLARATION
   --
   type Events_objU_Event_Type is new Events_Event_Type with
   record
   ...
   --    
   type Events_objU_Instance_Event_Type is new Events_objU_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type Events_objU_Description_Event is new Events_objU_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type Events_objU1 is new Events_objU_Instance_Event_Type with
   record 
      Test_Number : Application_Types.Base_Integer_Type;
   end record;

   type Events_objU2 is new Events_objU_Instance_Event_Type with
   record 
      Test_Number : Application_Types.Base_Integer_Type;
   end record;

   type Events_objT1 is new Events_objU_Instance_Event_Type with
   record 
      Test_Number : Application_Types.Base_Integer_Type;
   end record;
   ...
   package Events_objU_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   Events_objU_Queue: Events_objU_Event_Queue.FIFO_Type;
