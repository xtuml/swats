...
--* Description:             
--*                          Domain specification Root_Object.EventssbA
...
--  ----------------------------------  
--  package Root_Object.Events.sbA
...
package Root_Object.Events.sbA is
--
------------------------------------------------------------------------
   --
   --**************  Object SubA Information **************
   --
   -- Object sbA is ACTIVE
   --
   -- Object sbA participates in a super sub type relationship
   --
   --
   --
   -- Object sbA is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Idle,
   --   Update.
   --
   -- Object sbA is a SUB type of SprA via Relationship R13
   --
   -- Object sbA does not have an identifying non-referential attribute and 
   -- therefore may not be uniquely created.
   -- 
   -- Object sbA has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "sbA";

   --
   -------------------------------------------------------------------------------------------------
   --

   type Events_sbA_Type is new Events_Type with record
      ...
      R13_Super : Root_Object.Object_Access := null;
   ...
   procedure Put_R13_Super (
      This_Object : in  Root_Object.Object_Access;
      R13_Super_Value : in Root_Object.Object_Access);

   pragma inline (Get_R13_Super);
   pragma inline (Put_R13_Super);
   ...
   -- EVENT DECLARATION
   --
   type Events_sbA_Event_Type is new Events_Event_Type with
   record
   ...
   --    
   type Events_sbA_Instance_Event_Type is new Events_sbA_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type Events_sbA_Description_Event is new Events_sbA_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type Events_SprA1 is new Events_sbA_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;

   type Events_SprA2 is new Events_sbA_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;

   type Events_SprA3 is new Events_sbA_Instance_Event_Type with
   record 
      null;
   end record;

   type Events_SprA4 is new Events_sbA_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;

   type Events_SprA5 is new Events_sbA_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;

   type Events_SprA6 is new Events_sbA_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;
   ...
   package Events_sbA_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   Events_sbA_Queue: Events_sbA_Event_Queue.FIFO_Type;
