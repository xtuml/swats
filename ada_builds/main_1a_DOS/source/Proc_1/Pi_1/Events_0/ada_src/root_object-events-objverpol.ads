...
--*************************************************************************************
--*                                                                                   *
--* File Name:               r.Events.objVERPOL.ads
...
--* Description:             
--*                          Domain specification Root_Object.EventsobjVERPOL
...
--  ----------------------------------  
--  package Root_Object.Events.objVERPOL
...
package Root_Object.Events.objVERPOL is
--
------------------------------------------------------------------------
   --
   --**************  Object Object_Verify_Polymorphic Information **************
   --
   -- Object objVERPOL is ACTIVE
   --
   --
   --
   -- Object objVERPOL is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Idle,
   --   Verify_Event_Received,
   --   Fail.
   --
   -- Object objVERPOL has an identifying non-referential attribute ReferenceVER 
   -- and therefore may be uniquely created.
   -- 
   -- Object objVERPOL has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "objVERPOL";

   --
   -------------------------------------------------------------------------------------------------
   --

   type Events_objVERPOL_Type is new Events_Type with record

      --
      -- Non initialised identifying non referential
      ReferenceVER : Application_Types.Base_Integer_Type;

      --
      -- Initialised Non identifying non referential
      ResultSUPERTYPE : Application_Types.Base_Integer_Type := Application_Types.Base_Integer_Type_First;

      --
      -- Initialised Non identifying non referential
      ResultSUBTYPE : Application_Types.Base_Integer_Type := Application_Types.Base_Integer_Type_First;
   ...
   procedure Put_ReferenceVER (
      ...
      ReferenceVER_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_ReferenceVER);
   pragma inline (Put_ReferenceVER);
   ...
   procedure Put_ResultSUPERTYPE (
      This_Object : in Root_Object.Object_Access;
      ResultSUPERTYPE_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_ResultSUPERTYPE);
   pragma inline (Put_ResultSUPERTYPE);
   ...
   procedure Put_ResultSUBTYPE (
      This_Object : in Root_Object.Object_Access;
      ResultSUBTYPE_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_ResultSUBTYPE);
   pragma inline (Put_ResultSUBTYPE);
   ...
   -- EVENT DECLARATION
   --
   type Events_objVERPOL_Event_Type is new Events_Event_Type with
   record
   ...
   --    
   type Events_objVERPOL_Instance_Event_Type is new Events_objVERPOL_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type Events_objVERPOL_Description_Event is new Events_objVERPOL_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type Events_objVERPOL1 is new Events_objVERPOL_Instance_Event_Type with
   record 
      null;
   end record;

   type Events_objVERPOL2 is new Events_objVERPOL_Instance_Event_Type with
   record 
      null;
   end record;

   type Events_objVERPOL3 is new Events_objVERPOL_Instance_Event_Type with
   record 
   ...
   package Events_objVERPOL_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   Events_objVERPOL_Queue: Events_objVERPOL_Event_Queue.FIFO_Type;
