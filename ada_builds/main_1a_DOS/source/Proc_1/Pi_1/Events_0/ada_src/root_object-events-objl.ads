...
--* Description:             
--*                          Domain specification Root_Object.EventsobjL
...
--  ----------------------------------  
--  package Root_Object.Events.objL
...
package Root_Object.Events.objL is
--
------------------------------------------------------------------------
   --
   --**************  Object Object_L Information **************
   --
   -- Object objL is ACTIVE
   --
   --
   --
   -- Object objL is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Idle,
   --   Sub_sub_state.
   --
   -- Object objL has an identifying non-referential attribute ReferenceL 
   ...
   type Events_objL_Type is new Events_Type with record

      --
      -- Non initialised identifying non referential
      ReferenceL : Application_Types.Base_Integer_Type;

      --
      -- Initialised Non identifying non referential
      ResultL : Application_Types.Base_Integer_Type := Application_Types.Base_Integer_Type_First;
   ...
   procedure Put_ReferenceL (
      ...
      ReferenceL_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_ReferenceL);
   pragma inline (Put_ReferenceL);
   ...
   procedure Put_ResultL (
      This_Object : in Root_Object.Object_Access;
      ResultL_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_ResultL);
   pragma inline (Put_ResultL);
   ...
   -- EVENT DECLARATION
   --
   type Events_objL_Event_Type is new Events_Event_Type with
   record
   ...
   --    
   type Events_objL_Instance_Event_Type is new Events_objL_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type Events_objL_Description_Event is new Events_objL_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type Events_objL1 is new Events_objL_Instance_Event_Type with
   record 
      null;
   end record;
   ...
   package Events_objL_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   Events_objL_Queue: Events_objL_Event_Queue.FIFO_Type;
