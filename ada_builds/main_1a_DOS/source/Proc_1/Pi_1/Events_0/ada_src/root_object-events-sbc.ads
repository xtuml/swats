...
--* Description:             
--*                          Domain specification Root_Object.EventssbC
...
--  ----------------------------------  
--  package Root_Object.Events.sbC
...
package Root_Object.Events.sbC is
--
------------------------------------------------------------------------
   --
   --**************  Object SubC Information **************
   --
   -- Object sbC is ACTIVE
   --
   -- Object sbC participates in a super sub type relationship
   --
   --
   --
   -- Object sbC is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Idle,
   --   Update.
   --
   -- Object sbC is a SUB type of SprA via Relationship R14
   --
   -- Object sbC does not have an identifying non-referential attribute and 
   -- therefore may not be uniquely created.
   -- 
   -- Object sbC has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "sbC";

   --
   -------------------------------------------------------------------------------------------------
   --

   type Events_sbC_Type is new Events_Type with record
   ...
   -- EVENT DECLARATION
   --
   type Events_sbC_Event_Type is new Events_Event_Type with
   record
   ...
   --    
   type Events_sbC_Instance_Event_Type is new Events_sbC_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type Events_sbC_Description_Event is new Events_sbC_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type Events_SprA1 is new Events_sbC_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;

   type Events_SprA2 is new Events_sbC_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;

   type Events_SprA3 is new Events_sbC_Instance_Event_Type with
   record 
      null;
   end record;

   type Events_SprA4 is new Events_sbC_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;

   type Events_SprA5 is new Events_sbC_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;

   type Events_SprA6 is new Events_sbC_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;
   ...
   package Events_sbC_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   Events_sbC_Queue: Events_sbC_Event_Queue.FIFO_Type;
