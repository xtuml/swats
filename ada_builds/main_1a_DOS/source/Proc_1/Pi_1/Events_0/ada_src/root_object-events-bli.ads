...
--* Description:             
--*                          Domain specification Root_Object.EventsBLI
...
--  ----------------------------------  
--  package Root_Object.Events.BLI
...
package Root_Object.Events.BLI is
--
------------------------------------------------------------------------
   --
   --**************  Object Bottom_Level_I Information **************
   --
   -- Object BLI is PASSIVE
   --
   -- Object BLI participates in a super sub type relationship
   --
   --
   --
   -- Object BLI is a SUB type of SL_THREE_D via Relationship R16
   --
   -- Object BLI has an identifying non-referential attribute BLI_id 
   -- and therefore may be uniquely created.
   -- 
   -- Object BLI has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "BLI";

   --
   -------------------------------------------------------------------------------------------------
   --

   type Events_BLI_Type is new Events_Type with record

      --
      -- Non initialised identifying non referential
      BLI_id : Application_Types.Base_Integer_Type;
      ...
      R16_Super : Root_Object.Object_Access := null;
   ...
   procedure Put_BLI_id (
      ...
      BLI_id_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_BLI_id);
   pragma inline (Put_BLI_id);
   ...
   procedure Put_R16_Super (
      This_Object : in  Root_Object.Object_Access;
      R16_Super_Value : in Root_Object.Object_Access);

   pragma inline (Get_R16_Super);
   pragma inline (Put_R16_Super);
