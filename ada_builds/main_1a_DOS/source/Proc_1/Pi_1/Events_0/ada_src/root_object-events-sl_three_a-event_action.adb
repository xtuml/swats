...
--*************************************************************************************
--*                                                                                   *
--* File Name:               r.Events.SL_THREE_A.event_action.ada
...
--*  This domain object is part of a super sub relationship.
--*  Polymorphic Events must be handled to active sub types
--*
--*  ERROR HANDLING                                                                   *
--*  --------------                                                                   *
--*  None                                                                             *
...
--*  separate of Root_Object.EventsSL_THREE_A
--*     Event_Action
--*
--*************************************************************************************
--  MODIFICATION RECORD
...
separate (Root_Object.Events.SL_THREE_A)
...
begin
   ...
   This_Action := Events_SL_THREE_A_Instance_Event_Type(This_Event.all).This_Event_Class;
   This := Events_SL_THREE_A_Instance_Event_Type(This_Event.all).Target_Instance;


   -- Ensure that the source instance still exists. 
   if This /= null then
      --
      -- I'm a supertype - send it on to my subtype - need to iterate for all subtype groups
      --
      declare
         Subtype_Instance:   Root_Object.Object_Access;
      begin
         Events_Rel_R15.Navigate (
            From => This, 
            To   => Subtype_Instance);

         Polymorphic_Put ( 
            This_Object => Subtype_Instance.all, 
      ...
      --
      -- end of polymorphic bit
      --  
      This_Event_Dispatch_Matrix (This_Action, Events_SL_THREE_A_Type(This.all).Current_State) (This, This_Event);
      --
   end if;
