...
--* Description:             
--*                          Domain specification Root_Object.EventsobjJ
...
--  ----------------------------------  
--  package Root_Object.Events.objJ
...
package Root_Object.Events.objJ is
--
------------------------------------------------------------------------
   --
   --**************  Object Object_J Information **************
   --
   -- Object objJ is ACTIVE
   --
   -- Object objJ participates in a super sub type relationship
   --
   --
   --
   -- Object objJ is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Idle,
   --   Supertype_state.
   --
   -- Object objJ is a SUPER type of 
   --    objK
   --    via Relationship R7
   --
   -- Object objJ has an identifying non-referential attribute ReferenceJ 
   -- and therefore may be uniquely created.
   -- 
   -- Object objJ has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "objJ";

   --
   -------------------------------------------------------------------------------------------------
   --

   type Events_objJ_Type is new Events_Type with record
      ...
      ResultJ : Application_Types.Base_Integer_Type := Application_Types.Base_Integer_Type_First;

      --
      -- Initialised Non identifying non referential
      action_count : Application_Types.Base_Integer_Type := Application_Types.Base_Integer_Type_First;
      ...
      R7_Sub : Root_Object.Object_Access := null;
   ...
   procedure Put_ResultJ (
      This_Object : in Root_Object.Object_Access;
      ResultJ_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_ResultJ);
   pragma inline (Put_ResultJ);
   ...
   procedure Put_action_count (
      This_Object : in Root_Object.Object_Access;
      action_count_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_action_count);
   pragma inline (Put_action_count);
   ...
   procedure Put_R7_Sub (
      This_Object :   in  Root_Object.Object_Access;
      R7_Sub_Value : in Root_Object.Object_Access);

   pragma inline (Get_R7_Sub);
   pragma inline (Put_R7_Sub);
   ...
   -- EVENT DECLARATION
   --
   type Events_objJ_Event_Type is new Events_Event_Type with
   record
   ...
   --    
   type Events_objJ_Instance_Event_Type is new Events_objJ_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type Events_objJ_Description_Event is new Events_objJ_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type Events_objJ1 is new Events_objJ_Instance_Event_Type with
   record 
      Test_Number : Application_Types.Base_Integer_Type;
   end record;

   type Events_objJ2 is new Events_objJ_Instance_Event_Type with
   record 
      Test_Number : Application_Types.Base_Integer_Type;
   end record;
   ...
   package Events_objJ_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   Events_objJ_Queue: Events_objJ_Event_Queue.FIFO_Type;
