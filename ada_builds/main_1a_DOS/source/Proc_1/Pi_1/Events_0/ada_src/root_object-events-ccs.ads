...
--* Description:             
--*                          Domain specification Root_Object.EventsCCS
...
--  ----------------------------------  
--  package Root_Object.Events.CCS
...
package Root_Object.Events.CCS is
--
------------------------------------------------------------------------
   --
   --**************  Object Check_Current_States Information **************
   --
   -- Object CCS is ACTIVE
   --
   --
   --
   -- Object CCS is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Check.
   --
   -- Object CCS has an identifying non-referential attribute ccs_id 
   -- and therefore may be uniquely created.
   -- 
   -- Object CCS has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "CCS";

   --
   -------------------------------------------------------------------------------------------------
   --

   type Events_CCS_Type is new Events_Type with record

      --
      -- Non initialised identifying non referential
      ccs_id : Application_Types.Base_Integer_Type;
   ...
   procedure Put_ccs_id (
      ...
      ccs_id_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_ccs_id);
   pragma inline (Put_ccs_id);
   ...
   -- EVENT DECLARATION
   --
   type Events_CCS_Event_Type is new Events_Event_Type with
   record
   ...
   --    
   type Events_CCS_Instance_Event_Type is new Events_CCS_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type Events_CCS_Description_Event is new Events_CCS_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type Events_CCS1 is new Events_CCS_Instance_Event_Type with
   record 
   ...
   package Events_CCS_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   Events_CCS_Queue: Events_CCS_Event_Queue.FIFO_Type;
