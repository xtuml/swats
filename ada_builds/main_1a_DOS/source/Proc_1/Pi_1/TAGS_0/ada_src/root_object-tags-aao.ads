...
--* Description:             
--*                          Domain specification Root_Object.TAGSAAO
...
package Root_Object.TAGS.AAO is
--
------------------------------------------------------------------------
   --
   ...
   --   Expire.
   --
   -- Object AAO has an identifying non-referential attribute Unique_Identifier 
   -- and therefore may be uniquely created.
   --
   -- Object AAO has the following TAGS attached :
   --    SINGLETON 
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ...
   type TAGS_AAO_Type is new TAGS_Type with record
   ...
   -- EVENT DECLARATION
   --
   type TAGS_AAO_Event_Type is new TAGS_Event_Type with
   record
   ...
   --    
   type TAGS_AAO_Instance_Event_Type is new TAGS_AAO_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type TAGS_AAO_Description_Event is new TAGS_AAO_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type TAGS_AAO1 is new TAGS_AAO_Instance_Event_Type with
   record 
      Value_To_Add : Application_Types.Base_Integer_Type;
   end record;

   type TAGS_AAO2 is new TAGS_AAO_Instance_Event_Type with
   record 
      null;
   end record;

   type TAGS_AAO3 is new TAGS_AAO_Instance_Event_Type with
   record 
      null;
   end record;

   type TAGS_AAO4 is new TAGS_AAO_Instance_Event_Type with
   record 
      null;
   end record;
   ...
   package TAGS_AAO_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   TAGS_AAO_Queue: TAGS_AAO_Event_Queue.FIFO_Type;
