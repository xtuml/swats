...
--* Description:             
--*                          Domain specification Root_Object.TAGSSGR
...
package Root_Object.TAGS.SGR is
--
------------------------------------------------------------------------
   --
   --**************  Object Singleton_Glitch_Recovery Information **************
   --
   -- Object SGR is PASSIVE
   --
   --
   --
   -- Object SGR has an identifying non-referential attribute sgr_id 
   -- and therefore may be uniquely created.
   --
   -- Object SGR has the following TAGS attached :
   --    SINGLETON 
   --    NON_INITIALISED 
   --
   ------------------------------------------------------------------------
   ...
   Object_String : String := "SGR";

   --
   -------------------------------------------------------------------------------------------------
   --

   type TAGS_SGR_Type is new TAGS_Type with record

      --
      -- Non initialised identifying non referential
      sgr_id : Application_Types.Base_Integer_Type;

      --
      -- Non initialised Non identifying non referential
      An_Integer : Application_Types.Base_Integer_Type;
      ...
      --
      -- Non initialised Non identifying non referential
      A_Real : Application_Types.Base_Float_Type;

      --
      -- Non initialised Non identifying non referential
      Environment : TAGS_Domain_Types.Environment_Type;
   ...
   procedure Put_sgr_id (
      ...
      sgr_id_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_sgr_id);
   pragma inline (Put_sgr_id);
