...
--* Description:             
--*                          Domain specification Root_Object.TAGSMSGR
...
package Root_Object.TAGS.MSGR is
--
------------------------------------------------------------------------
   --
   --**************  Object Multiplicity_Single_Glitch_Recovery_Object Information **************
   --
   -- Object MSGR is PASSIVE
   --
   --
   --
   -- Object MSGR has an identifying non-referential attribute msgr_id 
   -- and therefore may be uniquely created.
   --
   -- Object MSGR has the following TAGS attached :
   --    ARRAY (1)
   --    NON_INITIALISED 
   --
   ------------------------------------------------------------------------
   ...
   Object_String : String := "MSGR";

   --
   -------------------------------------------------------------------------------------------------
   --

   type TAGS_MSGR_Type is new TAGS_Type with record

      --
      -- Non initialised identifying non referential
      msgr_id : Application_Types.Base_Integer_Type;

      --
      -- Non initialised Non identifying non referential
      Attribute_One : Application_Types.Base_Integer_Type;

      --
      -- Non initialised Non identifying non referential
      Attribute_Two : Application_Types.Base_Integer_Type;
   ...
   procedure Put_msgr_id (
      ...
      msgr_id_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_msgr_id);
   pragma inline (Put_msgr_id);
