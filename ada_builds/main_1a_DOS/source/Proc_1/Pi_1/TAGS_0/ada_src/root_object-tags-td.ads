...
--* Description:             
--*                          Domain specification Root_Object.TAGSTD
...
package Root_Object.TAGS.TD is
--
------------------------------------------------------------------------
   --
   --**************  Object Test_Data Information **************
   --
   -- Object TD is ACTIVE
   --
   --
   --
   -- Object TD is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Idle,
   --   Tests_Complete.
   --
   -- Object TD has an identifying non-referential attribute td_id 
   ...
   type TAGS_TD_Type is new TAGS_Type with record
      ...
      Number : Application_Types.Base_Integer_Type := Application_Types.Base_Integer_Type_First;

      --
      -- Initialised Non identifying non referential
      Environment : TAGS_Domain_Types.Environment_Type := TAGS_Domain_Types.Environment_Type_First;
   ...
   procedure Put_Number (
      This_Object : in Root_Object.Object_Access;
      Number_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_Number);
   pragma inline (Put_Number);


   function Get_Environment (
      This_Object : Root_Object.Object_Access) 
   return TAGS_Domain_Types.Environment_Type;

   procedure Put_Environment (
      This_Object : in Root_Object.Object_Access;
      Environment_Value : in TAGS_Domain_Types.Environment_Type);

   pragma inline (Get_Environment);
   pragma inline (Put_Environment);
   ...
   -- EVENT DECLARATION
   --
   type TAGS_TD_Event_Type is new TAGS_Event_Type with
   record
   ...
   --    
   type TAGS_TD_Instance_Event_Type is new TAGS_TD_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type TAGS_TD_Description_Event is new TAGS_TD_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type TAGS_TD1 is new TAGS_TD_Instance_Event_Type with
   record 
      null;
   end record;
   ...
   package TAGS_TD_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   TAGS_TD_Queue: TAGS_TD_Event_Queue.FIFO_Type;
