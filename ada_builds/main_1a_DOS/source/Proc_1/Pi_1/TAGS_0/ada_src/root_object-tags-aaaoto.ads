...
--* Description:             
--*                          Domain specification Root_Object.TAGSAAAOTO
...
--  ----------------------------------  
--  package Root_Object.TAGS.AAAOTO
...
package Root_Object.TAGS.AAAOTO is
--
------------------------------------------------------------------------
   --
   --**************  Object An_Active_Array_Of_Three_Objects Information **************
   --
   -- Object AAAOTO is ACTIVE
   --
   --
   --
   -- Object AAAOTO is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Idle,
   --   Do_Something,
   --   Delete_One_Self,
   --   More_Tests.
   --
   -- Object AAAOTO has an identifying non-referential attribute Unique_Identifier 
   -- and therefore may be uniquely created.
   --
   -- Object AAAOTO has the following TAGS attached :
   --    ARRAY (3)
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ...
   Object_String : String := "AAAOTO";

   --
   -------------------------------------------------------------------------------------------------
   --

   type TAGS_AAAOTO_Type is new TAGS_Type with record
      ...
      Result : Application_Types.Base_Integer_Type := Application_Types.Base_Integer_Type_First;
   ...
   procedure Put_Result (
      This_Object : in Root_Object.Object_Access;
      Result_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_Result);
   pragma inline (Put_Result);
   ...
   -- EVENT DECLARATION
   --
   type TAGS_AAAOTO_Event_Type is new TAGS_Event_Type with
   record
   ...
   --    
   type TAGS_AAAOTO_Instance_Event_Type is new TAGS_AAAOTO_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type TAGS_AAAOTO_Description_Event is new TAGS_AAAOTO_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type TAGS_AAAOTO1 is new TAGS_AAAOTO_Instance_Event_Type with
   record 
      Test_Number : Application_Types.Base_Integer_Type;
      Add : Application_Types.Base_Integer_Type;
      Purpose : Application_Types.Base_Text_Type;
      Expected_Result : Application_Types.Base_Integer_Type;
   end record;

   type TAGS_AAAOTO2 is new TAGS_AAAOTO_Instance_Event_Type with
   record 
      Final_Value : Application_Types.Base_Integer_Type;
      Test_Number : Application_Types.Base_Integer_Type;
      Purpose : Application_Types.Base_Text_Type;
      Tell_Tales :  Boolean;
   end record;

   type TAGS_AAAOTO3 is new TAGS_AAAOTO_Instance_Event_Type with
   record 
      null;
   end record;

   type TAGS_AAAOTO4 is new TAGS_AAAOTO_Instance_Event_Type with
   record 
      null;
   end record;
   ...
   package TAGS_AAAOTO_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   TAGS_AAAOTO_Queue: TAGS_AAAOTO_Event_Queue.FIFO_Type;
