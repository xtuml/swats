...
--* Description:             
--*                          Domain specification Root_Object.TAGSATSASSOC
...
--  ----------------------------------  
--  package Root_Object.TAGS.ATSASSOC
...
package Root_Object.TAGS.ATSASSOC is
--
------------------------------------------------------------------------
   --
   --**************  Object Active_Tagged_Single_Assoc Information **************
   --
   -- Object ATSASSOC is ACTIVE
   --
   --
   --
   -- Object ATSASSOC is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Idle,
   --   Blast_Off.
   --
   -- Object ATSASSOC does not have an identifying non-referential attribute and 
   -- therefore may not be uniquely created.
   --
   -- Object ATSASSOC has the following TAGS attached :
   --    ARRAY (9)
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ...
   Object_String : String := "ATSASSOC";

   --
   -------------------------------------------------------------------------------------------------
   --

   type TAGS_ATSASSOC_Type is new TAGS_Type with record

      --
      -- Non initialised identifying referential
      Assoc_Unique_Identifier : Application_Types.Base_Integer_Type;
   ...
   procedure Put_Assoc_Unique_Identifier (
      ...
      Assoc_Unique_Identifier_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_Assoc_Unique_Identifier);
   pragma inline (Put_Assoc_Unique_Identifier);
   ...
   procedure Put_Attribute_Three (
      This_Object : in Root_Object.Object_Access;
      Attribute_Three_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_Attribute_Three);
   pragma inline (Put_Attribute_Three);
   ...
   -- EVENT DECLARATION
   --
   type TAGS_ATSASSOC_Event_Type is new TAGS_Event_Type with
   record
   ...
   --    
   type TAGS_ATSASSOC_Instance_Event_Type is new TAGS_ATSASSOC_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type TAGS_ATSASSOC_Description_Event is new TAGS_ATSASSOC_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type TAGS_ATSASSOC1 is new TAGS_ATSASSOC_Instance_Event_Type with
   record 
      ATS_ASSOC_Test_No : Application_Types.Base_Integer_Type;
   end record;
   ...
   package TAGS_ATSASSOC_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   TAGS_ATSASSOC_Queue: TAGS_ATSASSOC_Event_Queue.FIFO_Type;
