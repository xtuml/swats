...
--* Description:             
--*                          Domain specification Root_Object.TAGSCHECK
...
--  ----------------------------------  
--  package Root_Object.TAGS.CHECK
...
package Root_Object.TAGS.CHECK is
--
------------------------------------------------------------------------
   --
   --**************  Object Check_On_Active_Object Information **************
   --
   -- Object CHECK is ACTIVE
   --
   --
   --
   -- Object CHECK is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Idle,
   --   Check_State,
   --   Self_Terminate.
   --
   -- Object CHECK has an identifying non-referential attribute check_id 
   -- and therefore may be uniquely created.
   -- 
   -- Object CHECK has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "CHECK";

   --
   -------------------------------------------------------------------------------------------------
   --

   type TAGS_CHECK_Type is new TAGS_Type with record

      --
      -- Non initialised identifying non referential
      check_id : Application_Types.Base_Integer_Type;
   ...
   procedure Put_check_id (
      ...
      check_id_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_check_id);
   pragma inline (Put_check_id);
   ...
   -- EVENT DECLARATION
   --
   type TAGS_CHECK_Event_Type is new TAGS_Event_Type with
   record
   ...
   --    
   type TAGS_CHECK_Instance_Event_Type is new TAGS_CHECK_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type TAGS_CHECK_Description_Event is new TAGS_CHECK_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type TAGS_CHECK1 is new TAGS_CHECK_Instance_Event_Type with
   record 
      Test_Number : Application_Types.Base_Integer_Type;
      Active_IH :  Root_Object.Object_Access;
      Testing_For : Application_Types.Base_Text_Type;
      Expected : Application_Types.Base_Integer_Type;
      Your_Presence_Is_Required :  Boolean;
      Call_To_Reset :  Boolean;
   end record;

   type TAGS_CHECK2 is new TAGS_CHECK_Instance_Event_Type with
   record 
      null;
   end record;

   type TAGS_CHECK3 is new TAGS_CHECK_Instance_Event_Type with
   record 
      null;
   end record;
   ...
   package TAGS_CHECK_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   TAGS_CHECK_Queue: TAGS_CHECK_Event_Queue.FIFO_Type;
