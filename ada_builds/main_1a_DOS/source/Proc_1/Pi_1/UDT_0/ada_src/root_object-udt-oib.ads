...
--* Description:             
--*                          TAGGED Domain specification Root_Object.UDTOIB
...
package Root_Object.UDT.OIB is
--
------------------------------------------------------------------------
   --
   --**************  Object Object_Into_Bridge Information **************
   --
   -- Object OIB is ACTIVE
   --
   --
   --
   -- Object OIB is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Idle,
   --   Testing.
   --
   -- Object OIB has an identifying non-referential attribute Unique_Identifier 
   -- and therefore may be uniquely created.
   -- 
   -- Object OIB has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "OIB";

   --
   -------------------------------------------------------------------------------------------------
   --

   type UDT_OIB_Type is new UDT_Type with record
      ...
      A_Number : UDT_Domain_Types.Subset_1_Of_Integer := UDT_Domain_Types.Subset_1_Of_Integer_First;

      --
      -- Initialised Non identifying non referential
      Some_Enumeration : UDT_Domain_Types.Simple_Enumeration_Type := UDT_Domain_Types.Simple_Enumeration_Type_First;
   ...
   procedure Put_A_Number (
      This_Object : in Root_Object.Object_Access;
      A_Number_Value : in UDT_Domain_Types.Subset_1_Of_Integer);

   pragma inline (Get_A_Number);
   pragma inline (Put_A_Number);
   ...
   procedure Put_Some_Enumeration (
      This_Object : in Root_Object.Object_Access;
      Some_Enumeration_Value : in UDT_Domain_Types.Simple_Enumeration_Type);

   pragma inline (Get_Some_Enumeration);
   pragma inline (Put_Some_Enumeration);
   ...
   -- EVENT DECLARATION
   --
   type UDT_OIB_Event_Type is new UDT_Event_Type with
   record
   ...
   --    
   type UDT_OIB_Instance_Event_Type is new UDT_OIB_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type UDT_OIB_Description_Event is new UDT_OIB_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type UDT_OIB1 is new UDT_OIB_Instance_Event_Type with
   record 
      Where_To_Start : Application_Types.Base_Integer_Type;
   end record;
   ...
   package UDT_OIB_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   UDT_OIB_Queue: UDT_OIB_Event_Queue.FIFO_Type;
