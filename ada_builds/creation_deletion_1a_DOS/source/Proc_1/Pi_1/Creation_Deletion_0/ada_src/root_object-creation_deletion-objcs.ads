...
--*************************************************************************************
--*                                                                                   *
--* File Name:               r.Creation_Deletion.objCS.ads
...
--* Description:             
--*                          Domain specification Root_Object.Creation_DeletionobjCS
...
--  ----------------------------------  
--  package Root_Object.Creation_Deletion.objCS
...
package Root_Object.Creation_Deletion.objCS is
--
------------------------------------------------------------------------
   --
   --**************  Object Creation_State Information **************
   --
   -- Object objCS is ACTIVE
   --
   --
   --
   -- Object objCS is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Instance_creation,
   ...
   --   Passed.
   --
   -- Object objCS has an identifying non-referential attribute ReferenceCS 
   -- and therefore may be uniquely created.
   -- 
   -- Object objCS has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "objCS";

   --
   -------------------------------------------------------------------------------------------------
   --

   type Creation_Deletion_objCS_Type is new Creation_Deletion_Type with record

      --
      -- Non initialised identifying non referential
      ReferenceCS : Application_Types.Base_Integer_Type;

      --
      -- Initialised Non identifying non referential
      ResultCS : Application_Types.Base_Integer_Type := Application_Types.Base_Integer_Type_First;
   ...
   procedure Put_ReferenceCS (
      ...
      ReferenceCS_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_ReferenceCS);
   pragma inline (Put_ReferenceCS);
   ...
   procedure Put_ResultCS (
      This_Object : in Root_Object.Object_Access;
      ResultCS_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_ResultCS);
   pragma inline (Put_ResultCS);
   ...
   -- EVENT DECLARATION
   --
   type Creation_Deletion_objCS_Event_Type is new Creation_Deletion_Event_Type with
   record
   ...
   --    
   type Creation_Deletion_objCS_Instance_Event_Type is new Creation_Deletion_objCS_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type Creation_Deletion_objCS_Description_Event is new Creation_Deletion_objCS_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type Creation_Deletion_objCS1 is new Creation_Deletion_objCS_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;

   type Creation_Deletion_objCS2 is new Creation_Deletion_objCS_Instance_Event_Type with
   record 
      null;
   end record;

   type Creation_Deletion_objCS3 is new Creation_Deletion_objCS_Instance_Event_Type with
   record 
      null;
   end record;

   type Creation_Deletion_objCS4 is new Creation_Deletion_objCS_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;

   type Creation_Deletion_objCS5 is new Creation_Deletion_objCS_Instance_Event_Type with
   record 
      null;
   end record;
   ...
   package Creation_Deletion_objCS_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   Creation_Deletion_objCS_Queue: Creation_Deletion_objCS_Event_Queue.FIFO_Type;
