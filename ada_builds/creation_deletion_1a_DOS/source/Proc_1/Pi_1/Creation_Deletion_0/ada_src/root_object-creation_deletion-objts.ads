...
--*************************************************************************************
--*                                                                                   *
--* File Name:               r.Creation_Deletion.objTS.ads
...
--* Description:             
--*                          Domain specification Root_Object.Creation_DeletionobjTS
...
--  ----------------------------------  
--  package Root_Object.Creation_Deletion.objTS
...
package Root_Object.Creation_Deletion.objTS is
--
------------------------------------------------------------------------
   --
   --**************  Object Termination_State Information **************
   --
   -- Object objTS is ACTIVE
   --
   --
   --
   -- Object objTS is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Idle,
   --   Deletion_in_a_terminal_state.
   --
   -- Object objTS has an identifying non-referential attribute ReferenceTS 
   -- and therefore may be uniquely created.
   -- 
   -- Object objTS has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "objTS";

   --
   -------------------------------------------------------------------------------------------------
   --

   type Creation_Deletion_objTS_Type is new Creation_Deletion_Type with record
   ...
   procedure Put_ReferenceTS (
      ...
      ReferenceTS_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_ReferenceTS);
   pragma inline (Put_ReferenceTS);
   ...
   -- EVENT DECLARATION
   --
   type Creation_Deletion_objTS_Event_Type is new Creation_Deletion_Event_Type with
   record
   ...
   --    
   type Creation_Deletion_objTS_Instance_Event_Type is new Creation_Deletion_objTS_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type Creation_Deletion_objTS_Description_Event is new Creation_Deletion_objTS_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type Creation_Deletion_objTS3 is new Creation_Deletion_objTS_Instance_Event_Type with
   record 
      null;
   end record;
   ...
   package Creation_Deletion_objTS_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   Creation_Deletion_objTS_Queue: Creation_Deletion_objTS_Event_Queue.FIFO_Type;
