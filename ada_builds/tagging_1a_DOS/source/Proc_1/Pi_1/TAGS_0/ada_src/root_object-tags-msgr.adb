...
--* Description:             
--*                          TAGGED Domain body Root_Object.TAGSMSGR
...
------------------------------------------------------------------------
--
package body Root_Object.TAGS.MSGR is
--
------------------------------------------------------------------------
--***********************  OOA Environment *****************************
------------------------------------------------------------------------
   ...
   The_Object_Array    : array (1 .. Maximum_Number_Of_Instances) of aliased TAGS_MSGR_Type;

   type Object_List_Type is
   record
...
------------------------------------------------------------------------
   ...
   msgr_id_Value : Application_Types.Base_Integer_Type := Application_Types.Base_Integer_Type_first;
--
--ADATEST IGNORE_ON
   function Obtain_Subsequent_msgr_id return Application_Types.Base_Integer_Type is
   begin
      msgr_id_Value :=
         Application_Types.Base_Integer_Type'succ (msgr_id_Value);
      return msgr_id_Value;
   end Obtain_Subsequent_msgr_id;
...
--ADATEST IGNORE_ON
   procedure Put_msgr_id (
      This_Object : in Root_Object.Object_Access;
      msgr_id_Value : in Application_Types.Base_Integer_Type) is
   begin
      TAGS_MSGR_Type (This_Object.all).msgr_id :=
         msgr_id_Value;
   end Put_msgr_id;
...
--ADATEST IGNORE_ON
   function Get_Attribute_One (
      This_Object: Root_Object.Object_Access) return Application_Types.Base_Integer_Type is
   begin
      return TAGS_MSGR_Type (This_Object.all).Attribute_One;
...
--ADATEST IGNORE_ON
   ...
   begin
      TAGS_MSGR_Type (This_Object.all).Attribute_One :=
         Attribute_One_Value;
...
--ADATEST IGNORE_ON
   function Get_Attribute_Two (
      This_Object: Root_Object.Object_Access) return Application_Types.Base_Integer_Type is
   begin
      return TAGS_MSGR_Type (This_Object.all).Attribute_Two;
...
--ADATEST IGNORE_ON
   ...
   begin
      TAGS_MSGR_Type (This_Object.all).Attribute_Two :=
         Attribute_Two_Value;
...
--ADATEST IGNORE_ON
   ...
   begin
      --
      -- Multiplicity Array of Non Initialised Objects
      ...
      --
      Traverse_Array:
      for i in 1 .. Maximum_Number_Of_Instances loop

         if not The_Object_Array(i).Created then -- only 'create' if this slot is free

            This_Object                 := The_Object_Array (i)'access;
            Object_List.Size            := Object_List.Size + 1;
            Object_List.Max_Count       := Object_List.Size;
            The_Object_Array(i).Created := TRUE; -- now, don't do it again
            --
         ...
         -- Bail if this_object is null
         exit Traverse_Array when This_Object /= null;

      end loop Traverse_Array;
...
---------------------------------------------------------------------
   ...
   begin
      ...
      TAGS_MSGR_Type(Old_Instance.all).Created := FALSE;
