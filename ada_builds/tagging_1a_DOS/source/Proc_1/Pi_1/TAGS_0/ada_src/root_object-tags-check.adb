...
--* Description:             
--*                          Domain body Root_Object.TAGSCHECK
...
------------------------------------------------------------------------
--
package body Root_Object.TAGS.CHECK is
--
------------------------------------------------------------------------
--***********************  OOA Environment *****************************
------------------------------------------------------------------------
...
------------------------------------------------------------------------
   ...
   check_id_Value : Application_Types.Base_Integer_Type := Application_Types.Base_Integer_Type_first;
--
--ADATEST IGNORE_ON
   function Obtain_Subsequent_check_id return Application_Types.Base_Integer_Type is
   begin
      check_id_Value :=
         Application_Types.Base_Integer_Type'succ (check_id_Value);
      return check_id_Value;
   end Obtain_Subsequent_check_id;
...
--ADATEST IGNORE_ON
   ...
   begin
       return TAGS_CHECK_Type (This_Object.all).check_id;
   end Get_check_id;
...
--ADATEST IGNORE_ON
   procedure Put_check_id (
      This_Object : in Root_Object.Object_Access;
      check_id_Value : in Application_Types.Base_Integer_Type) is
   begin
      TAGS_CHECK_Type (This_Object.all).check_id :=
         check_id_Value;
   end Put_check_id;
...
--ADATEST IGNORE_ON
   ...
   begin
      return TAGS_CHECK_Type (This_Object.all).Current_State;
...
--ADATEST IGNORE_ON
   ...
   begin
      TAGS_CHECK_Type (This_Object.all).Current_State :=
         Current_State_Value;
...
--ADATEST IGNORE_ON
   ...
   begin
      ...
      if Free_List.First_Entry = null then
         This_Object := new TAGS_CHECK_Type;
   
      else
         This_Object := Free_List.First_Entry;
         TAGS_CHECK_Type(This_Object.all).check_id := Application_Types.Base_Integer_Type_First;
         TAGS_CHECK_Type(This_Object.all).Current_State :=  Object_State_Type_First;
...
--
   ...
   procedure TAGS_CHECK_3 (
      This_Instance : in out Root_Object.Object_Access ) is separate;

   procedure TAGS_CHECK_1 (
      This_Instance : in out Root_Object.Object_Access ) is separate;


--
   procedure CHECK1_To_Check_State (
      This_Instance : in out Root_Object.Object_Access;
      This_Event    : in     Root_Object.Root_Event_Access_Type) is

   begin

      TAGS_CHECK_Type (This_Instance.all).Current_State := 
         Root_Object.TAGS.CHECK.Object_State_Type(Check_State);

      TAGS_CHECK_2 (
         Test_Number =>  TAGS_CHECK1(This_Event.all).Test_Number,
         Active_IH =>  TAGS_CHECK1(This_Event.all).Active_IH,
         Testing_For =>  TAGS_CHECK1(This_Event.all).Testing_For,
         Expected =>  TAGS_CHECK1(This_Event.all).Expected,
         Your_Presence_Is_Required =>  TAGS_CHECK1(This_Event.all).Your_Presence_Is_Required,
         Call_To_Reset =>  TAGS_CHECK1(This_Event.all).Call_To_Reset,
         This_Instance => This_Instance );

   end CHECK1_To_Check_State;
--
   procedure CHECK2_To_Self_Terminate (
      This_Instance : in out Root_Object.Object_Access;
      This_Event    : in     Root_Object.Root_Event_Access_Type) is
   ...
   begin
      ...
      TAGS_CHECK_Type (This_Instance.all).Current_State := 
         Root_Object.TAGS.CHECK.Object_State_Type(Self_Terminate);

      TAGS_CHECK_3 (
         This_Instance => This_Instance );

   end CHECK2_To_Self_Terminate;
--
   procedure CHECK3_To_Idle (
      This_Instance : in out Root_Object.Object_Access;
      This_Event    : in     Root_Object.Root_Event_Access_Type) is
   ...
   begin
      ...
      TAGS_CHECK_Type (This_Instance.all).Current_State := 
         Root_Object.TAGS.CHECK.Object_State_Type(Idle);
...
--ADATEST IGNORE_ON
   procedure Event_Action (
      Dispatch_Event : in     TAGS_CHECK_Event_Type;
...
--
   procedure Event_Action (
      Dispatch_Event : in     TAGS_CHECK_Instance_Event_Type;
...
--ADATEST IGNORE_ON
   ...
   begin
      ...
      TAGS_CHECK_Event_Queue.Push (
         Item        => Event,
         Target_FIFO => TAGS_CHECK_Queue,
         Top         => To_Top);
...
--ADATEST IGNORE_ON
   ...
   begin
      ...
      return TAGS_CHECK_Event_Queue.Queue_Length (TAGS_CHECK_Queue);
...
--
   This_Event_Dispatch_Matrix (CHECK1, Non_Existent) :=  Cannot_Happen_Action'access;
   This_Event_Dispatch_Matrix (CHECK2, Non_Existent) :=  Cannot_Happen_Action'access;
   This_Event_Dispatch_Matrix (CHECK3, Non_Existent) :=  Cannot_Happen_Action'access;
   This_Event_Dispatch_Matrix (CHECK1, Idle) := CHECK1_To_Check_State'access;
   This_Event_Dispatch_Matrix (CHECK2, Idle) :=  Ignore_Action'access;
   This_Event_Dispatch_Matrix (CHECK3, Idle) :=  Ignore_Action'access;
   This_Event_Dispatch_Matrix (CHECK1, Check_State) :=  Ignore_Action'access;
   This_Event_Dispatch_Matrix (CHECK2, Check_State) := CHECK2_To_Self_Terminate'access;
   This_Event_Dispatch_Matrix (CHECK3, Check_State) := CHECK3_To_Idle'access;
   This_Event_Dispatch_Matrix (CHECK1, Self_Terminate) :=  Ignore_Action'access;
   This_Event_Dispatch_Matrix (CHECK2, Self_Terminate) :=  Ignore_Action'access;
   This_Event_Dispatch_Matrix (CHECK3, Self_Terminate) :=  Ignore_Action'access;

end Root_Object.TAGS.CHECK;
