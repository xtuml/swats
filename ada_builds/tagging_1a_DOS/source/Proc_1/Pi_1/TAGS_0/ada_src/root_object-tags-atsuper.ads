...
--* Description:             
--*                          Domain specification Root_Object.TAGSATSUPER
...
--  ----------------------------------  
--  package Root_Object.TAGS.ATSUPER
...
package Root_Object.TAGS.ATSUPER is
--
------------------------------------------------------------------------
   --
   --**************  Object Active_Tagged_Supertype Information **************
   --
   -- Object ATSUPER is ACTIVE
   --
   -- Object ATSUPER participates in a super sub type relationship
   --
   --
   --
   -- Object ATSUPER is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Idle,
   --   Not_Idle.
   --
   -- Object ATSUPER is a SUPER type of 
   --    ATSUBONE,
   --    ATSUBTWO,
   --    ATSUBTHREE
   --    via Relationship R6
   --
   -- Object ATSUPER has an identifying non-referential attribute Unique_Identifier 
   -- and therefore may be uniquely created.
   --
   -- Object ATSUPER has the following TAGS attached :
   --    SINGLETON 
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ...
   Object_String : String := "ATSUPER";

   --
   -------------------------------------------------------------------------------------------------
   --

   type TAGS_ATSUPER_Type is new TAGS_Type with record
      ...
      Supertype_Data : Application_Types.Base_Integer_Type := Application_Types.Base_Integer_Type_First;
   ...
   procedure Put_Supertype_Data (
      This_Object : in Root_Object.Object_Access;
      Supertype_Data_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_Supertype_Data);
   pragma inline (Put_Supertype_Data);
   ...
   -- EVENT DECLARATION
   --
   type TAGS_ATSUPER_Event_Type is new TAGS_Event_Type with
   record
   ...
   --    
   type TAGS_ATSUPER_Instance_Event_Type is new TAGS_ATSUPER_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type TAGS_ATSUPER_Description_Event is new TAGS_ATSUPER_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type TAGS_ATSUPER1 is new TAGS_ATSUPER_Instance_Event_Type with
   record 
      null;
   end record;
   ...
   package TAGS_ATSUPER_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   TAGS_ATSUPER_Queue: TAGS_ATSUPER_Event_Queue.FIFO_Type;
