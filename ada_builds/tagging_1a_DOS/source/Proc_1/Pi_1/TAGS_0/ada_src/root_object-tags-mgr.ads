...
--* Description:             
--*                          Domain specification Root_Object.TAGSMGR
...
package Root_Object.TAGS.MGR is
--
------------------------------------------------------------------------
   --
   --**************  Object Multiplicity_Glitch_Recovery Information **************
   --
   -- Object MGR is PASSIVE
   --
   --
   --
   -- Object MGR has an identifying non-referential attribute mgr_id 
   -- and therefore may be uniquely created.
   --
   -- Object MGR has the following TAGS attached :
   --    ARRAY (3)
   --    NON_INITIALISED 
   --
   ------------------------------------------------------------------------
   ...
   Object_String : String := "MGR";

   --
   -------------------------------------------------------------------------------------------------
   --

   type TAGS_MGR_Type is new TAGS_Type with record

      --
      -- Non initialised identifying non referential
      mgr_id : Application_Types.Base_Integer_Type;
   ...
   procedure Put_mgr_id (
      ...
      mgr_id_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_mgr_id);
   pragma inline (Put_mgr_id);
