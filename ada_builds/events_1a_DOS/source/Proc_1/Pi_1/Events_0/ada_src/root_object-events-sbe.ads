...
--* Description:             
--*                          Domain specification Root_Object.EventssbE
...
--  ----------------------------------  
--  package Root_Object.Events.sbE
...
package Root_Object.Events.sbE is
--
------------------------------------------------------------------------
   --
   --**************  Object SubE Information **************
   --
   -- Object sbE is ACTIVE
   --
   -- Object sbE participates in a super sub type relationship
   --
   --
   --
   -- Object sbE is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Idle,
   --   Update.
   --
   -- Object sbE is a SUB type of SprA via Relationship R6
   --
   -- Object sbE does not have an identifying non-referential attribute and 
   -- therefore may not be uniquely created.
   -- 
   -- Object sbE has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "sbE";

   --
   -------------------------------------------------------------------------------------------------
   --

   type Events_sbE_Type is new Events_Type with record
   ...
   -- EVENT DECLARATION
   --
   type Events_sbE_Event_Type is new Events_Event_Type with
   record
   ...
   --    
   type Events_sbE_Instance_Event_Type is new Events_sbE_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type Events_sbE_Description_Event is new Events_sbE_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type Events_SprA1 is new Events_sbE_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;

   type Events_SprA2 is new Events_sbE_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;

   type Events_SprA3 is new Events_sbE_Instance_Event_Type with
   record 
      null;
   end record;

   type Events_SprA4 is new Events_sbE_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;

   type Events_SprA5 is new Events_sbE_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;

   type Events_SprA6 is new Events_sbE_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;
   ...
   package Events_sbE_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   Events_sbE_Queue: Events_sbE_Event_Queue.FIFO_Type;
