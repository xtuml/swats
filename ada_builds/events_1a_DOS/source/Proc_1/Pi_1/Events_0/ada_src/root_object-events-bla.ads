...
--* Description:             
--*                          Domain specification Root_Object.EventsBLA
...
--  ----------------------------------  
--  package Root_Object.Events.BLA
...
package Root_Object.Events.BLA is
--
------------------------------------------------------------------------
   --
   --**************  Object Bottom_Level_A Information **************
   --
   -- Object BLA is PASSIVE
   --
   -- Object BLA participates in a super sub type relationship
   --
   --
   --
   -- Object BLA is a SUB type of SL_THREE_A via Relationship R15
   --
   -- Object BLA has an identifying non-referential attribute BLA_id 
   -- and therefore may be uniquely created.
   -- 
   -- Object BLA has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "BLA";

   --
   -------------------------------------------------------------------------------------------------
   --

   type Events_BLA_Type is new Events_Type with record

      --
      -- Non initialised identifying non referential
      BLA_id : Application_Types.Base_Integer_Type;
      ...
      R15_Super : Root_Object.Object_Access := null;
   ...
   procedure Put_BLA_id (
      ...
      BLA_id_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_BLA_id);
   pragma inline (Put_BLA_id);
   ...
   procedure Put_R15_Super (
      This_Object : in  Root_Object.Object_Access;
      R15_Super_Value : in Root_Object.Object_Access);

   pragma inline (Get_R15_Super);
   pragma inline (Put_R15_Super);
