...
--* Description:             
--*                          Domain specification Root_Object.EventsobjMCS
...
--  ----------------------------------  
--  package Root_Object.Events.objMCS
...
package Root_Object.Events.objMCS is
--
------------------------------------------------------------------------
   --
   --**************  Object Multiple_Creation_States Information **************
   --
   -- Object objMCS is ACTIVE
   --
   --
   --
   -- Object objMCS is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Creation_stateA,
   --   Creation_stateB,
   --   Idle,
   ...
   --   Fail.
   --
   -- Object objMCS has an identifying non-referential attribute idMCS 
   -- and therefore may be uniquely created.
   -- 
   -- Object objMCS has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "objMCS";

   --
   -------------------------------------------------------------------------------------------------
   --

   type Events_objMCS_Type is new Events_Type with record

      --
      -- Non initialised identifying non referential
      idMCS : Application_Types.Base_Integer_Type;
   ...
   procedure Put_idMCS (
      ...
      idMCS_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_idMCS);
   pragma inline (Put_idMCS);
   ...
   -- EVENT DECLARATION
   --
   type Events_objMCS_Event_Type is new Events_Event_Type with
   record
   ...
   --    
   type Events_objMCS_Instance_Event_Type is new Events_objMCS_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type Events_objMCS_Description_Event is new Events_objMCS_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type Events_objMCS1 is new Events_objMCS_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;

   type Events_objMCS2 is new Events_objMCS_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;

   type Events_objMCS3 is new Events_objMCS_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;

   type Events_objMCS4 is new Events_objMCS_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
      Test_Number : Application_Types.Base_Integer_Type;
   end record;

   type Events_objMCS5 is new Events_objMCS_Instance_Event_Type with
   record 
      null;
   end record;

   type Events_objMCS6 is new Events_objMCS_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;
   ...
   package Events_objMCS_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   Events_objMCS_Queue: Events_objMCS_Event_Queue.FIFO_Type;
