...
--* Description:             
--*                          Domain specification Root_Object.EventsobjR
...
--  ----------------------------------  
--  package Root_Object.Events.objR
...
package Root_Object.Events.objR is
--
------------------------------------------------------------------------
   --
   --**************  Object Object_R Information **************
   --
   -- Object objR is ACTIVE
   --
   -- Object objR participates in a super sub type relationship
   --
   --
   --
   -- Object objR is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Idle,
   --   Subtype_State.
   --
   -- Object objR is a SUB type of objP via Relationship R4
   --
   -- Object objR does not have an identifying non-referential attribute and 
   -- therefore may not be uniquely created.
   -- 
   -- Object objR has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "objR";

   --
   -------------------------------------------------------------------------------------------------
   --

   type Events_objR_Type is new Events_Type with record
      ...
      --
      -- Initialised Non identifying non referential
      Result_R : Application_Types.Base_Integer_Type := Application_Types.Base_Integer_Type_First;

      --
      -- Initialised Non identifying non referential
      Current_State :  Object_State_Type :=  Object_State_Type_First;

      R4_Super : Root_Object.Object_Access := null;
   ...
   procedure Put_Result_R (
      This_Object : in Root_Object.Object_Access;
      Result_R_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_Result_R);
   pragma inline (Put_Result_R);
   ...
   procedure Put_R4_Super (
      This_Object : in  Root_Object.Object_Access;
      R4_Super_Value : in Root_Object.Object_Access);

   pragma inline (Get_R4_Super);
   pragma inline (Put_R4_Super);
   ...
   -- EVENT DECLARATION
   --
   type Events_objR_Event_Type is new Events_Event_Type with
   record
   ...
   --    
   type Events_objR_Instance_Event_Type is new Events_objR_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type Events_objR_Description_Event is new Events_objR_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type Events_objR1 is new Events_objR_Instance_Event_Type with
   record 
      null;
   end record;

   type Events_objP1 is new Events_objR_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;

   type Events_objP2 is new Events_objR_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;
   ...
   package Events_objR_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   Events_objR_Queue: Events_objR_Event_Queue.FIFO_Type;
