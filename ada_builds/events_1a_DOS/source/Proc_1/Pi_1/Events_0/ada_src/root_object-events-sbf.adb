...
--* Description:             
--*                          Domain body Root_Object.EventssbF
...
------------------------------------------------------------------------
--
package body Root_Object.Events.sbF is
--
------------------------------------------------------------------------
--***********************  OOA Environment *****************************
------------------------------------------------------------------------
...
--ADATEST IGNORE_ON
   ...
   begin
      return Events_sbF_Type (This_Object.all).idA;
   end Get_idA;
--ADATEST IGNORE_OFF
...
--ADATEST IGNORE_ON
   ...
   begin
      Events_sbF_Type (This_Object.all).idA :=
         idA_Value;
   end Put_idA;
--ADATEST IGNORE_OFF
...
--ADATEST IGNORE_ON
   function Get_Value_To_Add (
      This_Object: Root_Object.Object_Access) return Application_Types.Base_Integer_Type is
   begin
      return Events_sbF_Type (This_Object.all).Value_To_Add;
...
--ADATEST IGNORE_ON
   ...
   begin
      Events_sbF_Type (This_Object.all).Value_To_Add :=
         Value_To_Add_Value;
...
--ADATEST IGNORE_ON
   ...
   begin
      return Events_sbF_Type (This_Object.all).Current_State;
...
--ADATEST IGNORE_ON
   ...
   begin
      Events_sbF_Type (This_Object.all).Current_State :=
         Current_State_Value;
...
--ADATEST IGNORE_ON
   ...
   begin
      return Events_sbF_Type (This_Object.all).R6_Super;
   end Get_R6_Super;
--ADATEST IGNORE_OFF
...
--ADATEST IGNORE_ON
   ...
   begin
      Events_sbF_Type (This_Object.all).R6_Super :=
         R6_Super_Value;
   end Put_R6_Super;
--ADATEST IGNORE_OFF
...
--ADATEST IGNORE_ON
   ...
   begin
      ...
      if Free_List.First_Entry = null then
         This_Object := new Events_sbF_Type;
   
      else
         This_Object := Free_List.First_Entry;
         Events_sbF_Type(This_Object.all).idA := Application_Types.Base_Integer_Type_First;
         Events_sbF_Type(This_Object.all).Value_To_Add := Application_Types.Base_Integer_Type_First;
         Events_sbF_Type(This_Object.all).Current_State :=  Object_State_Type_First;
...
----------------------------------------------------------------------------
   ...
   procedure Polymorphic_Put (
      This_Object : in out Events_sbF_Type;
   ...
   begin
      ...
      case Parent_Event_Type is
         ...
         when Root_Object.Events.SprA.SprA1 =>
            ...
            Next_Event := new Root_Object.Events.sbF.Events_SprA1'(
            Root_Event_Type with
               Test =>
                  Root_Object.Events.SprA.Events_SprA1(This_Event.all).Test,
               This_Event_Class => Root_Object.Events.sbF.SprA1,
               Target_Instance  => This_Object'Unchecked_Access);

         when Root_Object.Events.SprA.SprA2 =>

            Next_Event := new Root_Object.Events.sbF.Events_SprA2'(
            Root_Event_Type with
               Test =>
                  Root_Object.Events.SprA.Events_SprA2(This_Event.all).Test,
               This_Event_Class => Root_Object.Events.sbF.SprA2,
               Target_Instance  => This_Object'Unchecked_Access);

         when Root_Object.Events.SprA.SprA3 =>

            Next_Event := new Root_Object.Events.sbF.Events_SprA3'(
            Root_Event_Type with
               This_Event_Class => Root_Object.Events.sbF.SprA3,
               Target_Instance  => This_Object'Unchecked_Access);

         when Root_Object.Events.SprA.SprA4 =>

            Next_Event := new Root_Object.Events.sbF.Events_SprA4'(
            Root_Event_Type with
               Test =>
                  Root_Object.Events.SprA.Events_SprA4(This_Event.all).Test,
               This_Event_Class => Root_Object.Events.sbF.SprA4,
               Target_Instance  => This_Object'Unchecked_Access);

         when Root_Object.Events.SprA.SprA5 =>

            Next_Event := new Root_Object.Events.sbF.Events_SprA5'(
            Root_Event_Type with
               Test =>
                  Root_Object.Events.SprA.Events_SprA5(This_Event.all).Test,
               This_Event_Class => Root_Object.Events.sbF.SprA5,
               Target_Instance  => This_Object'Unchecked_Access);

         when Root_Object.Events.SprA.SprA6 =>

            Next_Event := new Root_Object.Events.sbF.Events_SprA6'(
            Root_Event_Type with
               Test =>
                  Root_Object.Events.SprA.Events_SprA6(This_Event.all).Test,
               This_Event_Class => Root_Object.Events.sbF.SprA6,
               Target_Instance  => This_Object'Unchecked_Access);
...
--
   ...
   begin
      ...
      Events_sbF_Type (This_Instance.all).Current_State := 
         Root_Object.Events.sbF.Object_State_Type(Update);
...
--ADATEST IGNORE_ON
   procedure Event_Action (
      Dispatch_Event : in     Events_sbF_Event_Type;
...
--
   procedure Event_Action (
      Dispatch_Event : in     Events_sbF_Instance_Event_Type;
...
--ADATEST IGNORE_ON
   ...
   begin
      ...
      Events_sbF_Event_Queue.Push (
         Item        => Event,
         Target_FIFO => Events_sbF_Queue,
         Top         => To_Top);
...
--ADATEST IGNORE_ON
   ...
   begin
      ...
      return Events_sbF_Event_Queue.Queue_Length (Events_sbF_Queue);
