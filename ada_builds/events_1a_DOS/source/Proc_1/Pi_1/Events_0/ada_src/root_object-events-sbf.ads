...
--* Description:             
--*                          Domain specification Root_Object.EventssbF
...
--  ----------------------------------  
--  package Root_Object.Events.sbF
...
package Root_Object.Events.sbF is
--
------------------------------------------------------------------------
   --
   --**************  Object SubF Information **************
   --
   -- Object sbF is ACTIVE
   --
   -- Object sbF participates in a super sub type relationship
   --
   --
   --
   -- Object sbF is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Idle,
   --   Update.
   --
   -- Object sbF is a SUB type of SprA via Relationship R6
   --
   -- Object sbF does not have an identifying non-referential attribute and 
   -- therefore may not be uniquely created.
   -- 
   -- Object sbF has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "sbF";

   --
   -------------------------------------------------------------------------------------------------
   --

   type Events_sbF_Type is new Events_Type with record
   ...
   -- EVENT DECLARATION
   --
   type Events_sbF_Event_Type is new Events_Event_Type with
   record
   ...
   --    
   type Events_sbF_Instance_Event_Type is new Events_sbF_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type Events_sbF_Description_Event is new Events_sbF_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type Events_SprA1 is new Events_sbF_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;

   type Events_SprA2 is new Events_sbF_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;

   type Events_SprA3 is new Events_sbF_Instance_Event_Type with
   record 
      null;
   end record;

   type Events_SprA4 is new Events_sbF_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;

   type Events_SprA5 is new Events_sbF_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;

   type Events_SprA6 is new Events_sbF_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;
   ...
   package Events_sbF_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   Events_sbF_Queue: Events_sbF_Event_Queue.FIFO_Type;
