...
--* Description:             
--*                          Domain specification Root_Object.EventsHSM
...
--  ----------------------------------  
--  package Root_Object.Events.HSM
...
package Root_Object.Events.HSM is
--
------------------------------------------------------------------------
   --
   --**************  Object Huge_State_Machine Information **************
   --
   -- Object HSM is ACTIVE
   --
   --
   --
   -- Object HSM is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Idle,
   --   State_A,
   --   State_B,
   ...
   --   State_Z.
   --
   -- Object HSM has an identifying non-referential attribute hsm_id 
   -- and therefore may be uniquely created.
   -- 
   -- Object HSM has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "HSM";

   --
   -------------------------------------------------------------------------------------------------
   --

   type Events_HSM_Type is new Events_Type with record

      --
      -- Non initialised identifying non referential
      hsm_id : Application_Types.Base_Integer_Type;
   ...
   procedure Put_hsm_id (
      ...
      hsm_id_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_hsm_id);
   pragma inline (Put_hsm_id);
   ...
   -- EVENT DECLARATION
   --
   type Events_HSM_Event_Type is new Events_Event_Type with
   record
   ...
   --    
   type Events_HSM_Instance_Event_Type is new Events_HSM_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type Events_HSM_Description_Event is new Events_HSM_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type Events_HSM1 is new Events_HSM_Instance_Event_Type with
   record 
      Test_No : Application_Types.Base_Integer_Type;
      Count : Application_Types.Base_Integer_Type;
   end record;
   ...
   package Events_HSM_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   Events_HSM_Queue: Events_HSM_Event_Queue.FIFO_Type;
