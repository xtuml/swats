...
--* Description:             
--*                          Domain specification Root_Object.EventsobjT
...
--  ----------------------------------  
--  package Root_Object.Events.objT
...
package Root_Object.Events.objT is
--
------------------------------------------------------------------------
   --
   --**************  Object Object_T Information **************
   --
   -- Object objT is PASSIVE
   --
   -- Object objT participates in a super sub type relationship
   --
   --
   --
   -- Object objT is a SUPER type of 
   --    objU
   --    via Relationship R8
   --
   -- Object objT has an identifying non-referential attribute Reference_T 
   -- and therefore may be uniquely created.
   -- 
   -- Object objT has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "objT";

   --
   -------------------------------------------------------------------------------------------------
   --

   type Events_objT_Type is new Events_Type with record
      ...
      Result_T : Application_Types.Base_Integer_Type := Application_Types.Base_Integer_Type_First;

      --
      -- Initialised Non identifying non referential
      Current_State :  Object_State_Type :=  Object_State_Type_First;

      R8_Sub : Root_Object.Object_Access := null;
   ...
   procedure Put_Result_T (
      This_Object : in Root_Object.Object_Access;
      Result_T_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_Result_T);
   pragma inline (Put_Result_T);
   ...
   procedure Put_R8_Sub (
      This_Object :   in  Root_Object.Object_Access;
      R8_Sub_Value : in Root_Object.Object_Access);

   pragma inline (Get_R8_Sub);
   pragma inline (Put_R8_Sub);
   ...
   --
   type Events_objT_Event_Type is new Events_Event_Type with
   record
   ...
   --    
   type Events_objT_Instance_Event_Type is new Events_objT_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type Events_objT_Description_Event is new Events_objT_Instance_Event_Type with
   record
      null;
   end record;
   --
   ...
   --
   --
   type Events_objT1 is new Events_objT_Instance_Event_Type with
   record 
