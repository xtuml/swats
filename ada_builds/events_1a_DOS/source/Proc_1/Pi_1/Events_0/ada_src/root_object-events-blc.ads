...
--* Description:             
--*                          Domain specification Root_Object.EventsBLC
...
--  ----------------------------------  
--  package Root_Object.Events.BLC
...
package Root_Object.Events.BLC is
--
------------------------------------------------------------------------
   --
   --**************  Object Bottom_Level_C Information **************
   --
   -- Object BLC is PASSIVE
   --
   -- Object BLC participates in a super sub type relationship
   --
   --
   --
   -- Object BLC is a SUB type of SL_THREE_A via Relationship R18
   --
   -- Object BLC has an identifying non-referential attribute BLC_Identifier 
   -- and therefore may be uniquely created.
   -- 
   -- Object BLC has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "BLC";

   --
   -------------------------------------------------------------------------------------------------
   --

   type Events_BLC_Type is new Events_Type with record

      --
      -- Non initialised identifying non referential
      BLC_Identifier : Application_Types.Base_Integer_Type;
      ...
      R18_Super : Root_Object.Object_Access := null;
   ...
   procedure Put_BLC_Identifier (
      ...
      BLC_Identifier_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_BLC_Identifier);
   pragma inline (Put_BLC_Identifier);
   ...
   procedure Put_R18_Super (
      This_Object : in  Root_Object.Object_Access;
      R18_Super_Value : in Root_Object.Object_Access);

   pragma inline (Get_R18_Super);
   pragma inline (Put_R18_Super);
