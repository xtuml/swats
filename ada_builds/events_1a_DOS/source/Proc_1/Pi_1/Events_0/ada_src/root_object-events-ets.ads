...
--* Description:             
--*                          Domain specification Root_Object.EventsETS
...
--  ----------------------------------  
--  package Root_Object.Events.ETS
...
package Root_Object.Events.ETS is
--
------------------------------------------------------------------------
   --
   --**************  Object EventToSelf Information **************
   --
   -- Object ETS is ACTIVE
   --
   --
   --
   -- Object ETS is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Idle,
   --   State_One,
   --   State_Two,
   --   Verify_event_to_self,
   --   Fail.
   --
   -- Object ETS has an identifying non-referential attribute idETS 
   -- and therefore may be uniquely created.
   -- 
   -- Object ETS has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "ETS";

   --
   -------------------------------------------------------------------------------------------------
   --

   type Events_ETS_Type is new Events_Type with record

      --
      -- Non initialised identifying non referential
      idETS : Application_Types.Base_Integer_Type;

      --
      -- Initialised Non identifying non referential
      NewResult : Application_Types.Base_Integer_Type := Application_Types.Base_Integer_Type_First;

      --
      -- Initialised Non identifying non referential
      OldResult : Application_Types.Base_Integer_Type := Application_Types.Base_Integer_Type_First;
   ...
   procedure Put_idETS (
      ...
      idETS_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_idETS);
   pragma inline (Put_idETS);
   ...
   procedure Put_NewResult (
      This_Object : in Root_Object.Object_Access;
      NewResult_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_NewResult);
   pragma inline (Put_NewResult);
   ...
   procedure Put_OldResult (
      This_Object : in Root_Object.Object_Access;
      OldResult_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_OldResult);
   pragma inline (Put_OldResult);
   ...
   -- EVENT DECLARATION
   --
   type Events_ETS_Event_Type is new Events_Event_Type with
   record
   ...
   --    
   type Events_ETS_Instance_Event_Type is new Events_ETS_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type Events_ETS_Description_Event is new Events_ETS_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type Events_ETS1 is new Events_ETS_Instance_Event_Type with
   record 
      null;
   end record;

   type Events_ETS2 is new Events_ETS_Instance_Event_Type with
   record 
      null;
   end record;

   type Events_ETS3 is new Events_ETS_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;

   type Events_ETS4 is new Events_ETS_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;

   type Events_ETS5 is new Events_ETS_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   ...
   package Events_ETS_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   Events_ETS_Queue: Events_ETS_Event_Queue.FIFO_Type;
