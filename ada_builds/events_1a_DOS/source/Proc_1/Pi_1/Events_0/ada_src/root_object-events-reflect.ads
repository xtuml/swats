...
--*************************************************************************************
--*                                                                                   *
--* File Name:               r.Events.REFLECT.ads
...
--* Description:             
--*                          Domain specification Root_Object.EventsREFLECT
...
--  ----------------------------------  
--  package Root_Object.Events.REFLECT
...
package Root_Object.Events.REFLECT is
--
------------------------------------------------------------------------
   --
   --**************  Object Reflexive Information **************
   --
   -- Object REFLECT is ACTIVE
   --
   --
   --
   -- Object REFLECT is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Idle,
   --   Test_state,
   --   Fail.
   --
   -- Object REFLECT has an identifying non-referential attribute ReferenceREFLECT 
   -- and therefore may be uniquely created.
   -- 
   -- Object REFLECT has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "REFLECT";

   --
   -------------------------------------------------------------------------------------------------
   --

   type Events_REFLECT_Type is new Events_Type with record
      ...
      ReferenceREFLECT : Application_Types.Base_Integer_Type;
   ...
   procedure Put_ReferenceREFLECT (
      This_Object : in Root_Object.Object_Access;
      ReferenceREFLECT_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_ReferenceREFLECT);
   pragma inline (Put_ReferenceREFLECT);
   ...
   -- EVENT DECLARATION
   --
   type Events_REFLECT_Event_Type is new Events_Event_Type with
   record
   ...
   --    
   type Events_REFLECT_Instance_Event_Type is new Events_REFLECT_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type Events_REFLECT_Description_Event is new Events_REFLECT_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type Events_REFLECT1 is new Events_REFLECT_Instance_Event_Type with
   record 
      null;
   end record;

   type Events_REFLECT2 is new Events_REFLECT_Instance_Event_Type with
   record 
      null;
   end record;

   type Events_REFLECT3 is new Events_REFLECT_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
      Counter : Application_Types.Base_Integer_Type;
   ...
   package Events_REFLECT_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   Events_REFLECT_Queue: Events_REFLECT_Event_Queue.FIFO_Type;
