...
--*************************************************************************************
--*                                                                                   *
--* File Name:               r.Events.SL_THREE_D.ads
...
--* Description:             
--*                          Domain specification Root_Object.EventsSL_THREE_D
...
--  ----------------------------------  
--  package Root_Object.Events.SL_THREE_D
...
package Root_Object.Events.SL_THREE_D is
--
------------------------------------------------------------------------
   --
   --**************  Object Sub_Level_3_D Information **************
   --
   -- Object SL_THREE_D is PASSIVE
   --
   -- Object SL_THREE_D participates in a super sub type relationship
   --
   --
   --
   -- Object SL_THREE_D is a SUPER type of 
   --    BLI,
   --    BLJ
   --    via Relationship R16
   --
   -- Object SL_THREE_D is a SUB type of SL_TWO_D via Relationship R17
   --
   -- Object SL_THREE_D has an identifying non-referential attribute SL3D_id 
   -- and therefore may be uniquely created.
   -- 
   -- Object SL_THREE_D has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "SL_THREE_D";

   --
   -------------------------------------------------------------------------------------------------
   --

   type Events_SL_THREE_D_Type is new Events_Type with record

      --
      -- Non initialised identifying non referential
      SL3D_id : Application_Types.Base_Integer_Type;

      --
      -- Non Identifying referential. Thou Shalt Not Use. 
      -- SL2D_id : Application_Types.Base_Integer_Type;
   
      --
      -- Initialised Non identifying non referential
      Current_State :  Object_State_Type :=  Object_State_Type_First;

      R16_Sub : Root_Object.Object_Access := null;
      R17_Super : Root_Object.Object_Access := null;
   ...
   function Get_SL3D_id (
      This_Object : Root_Object.Object_Access) 
   return Application_Types.Base_Integer_Type;

   procedure Put_SL3D_id (
      This_Object : in Root_Object.Object_Access;
      SL3D_id_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_SL3D_id);
   pragma inline (Put_SL3D_id);
   ...
   function  Get_R16_Sub (
      This_Object : Root_Object.Object_Access) 
   return Root_Object.Object_Access;

   procedure Put_R16_Sub (
      This_Object :   in  Root_Object.Object_Access;
      R16_Sub_Value : in Root_Object.Object_Access);

   pragma inline (Get_R16_Sub);
   pragma inline (Put_R16_Sub);


   function  Get_R17_Super (
      This_Object : Root_Object.Object_Access) 
   return Root_Object.Object_Access;

   procedure Put_R17_Super (
      This_Object : in  Root_Object.Object_Access;
      R17_Super_Value : in Root_Object.Object_Access);

   pragma inline (Get_R17_Super);
   pragma inline (Put_R17_Super);
   ...
   --
   type Events_SL_THREE_D_Event_Type is new Events_Event_Type with
   record
      This_Event_Class : Event_Class;
   end record;

   procedure Event_Action (
      Dispatch_Event : in     Events_SL_THREE_D_Event_Type;
      This_Event     : in out Root_Object.Root_Event_Access_Type);


   --    
   type Events_SL_THREE_D_Instance_Event_Type is new Events_SL_THREE_D_Event_Type with
   record
   ...
   -- 
   procedure Event_Action (
      Dispatch_Event : in     Events_SL_THREE_D_Instance_Event_Type;
      This_Event     : in out Root_Object.Root_Event_Access_Type);



   ------------------------------------------------------------------------
   type Events_SL_THREE_D_Description_Event is new Events_SL_THREE_D_Instance_Event_Type with
   record
      null;
   end record;
   --
   ...
   --
   --
   type Events_SL_THREE_D1 is new Events_SL_THREE_D_Instance_Event_Type with
   record 
      null;
   end record;

end Root_Object.Events.SL_THREE_D;
