...
--* Description:             
--*                          Domain specification Root_Object.EventssbD
...
--  ----------------------------------  
--  package Root_Object.Events.sbD
...
package Root_Object.Events.sbD is
--
------------------------------------------------------------------------
   --
   --**************  Object SubD Information **************
   --
   -- Object sbD is ACTIVE
   --
   -- Object sbD participates in a super sub type relationship
   --
   --
   --
   -- Object sbD is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Idle,
   --   Update.
   --
   -- Object sbD is a SUB type of SprA via Relationship R14
   --
   -- Object sbD does not have an identifying non-referential attribute and 
   ...
   type Events_sbD_Type is new Events_Type with record
      ...
      R14_Super : Root_Object.Object_Access := null;
   ...
   procedure Put_R14_Super (
      This_Object : in  Root_Object.Object_Access;
      R14_Super_Value : in Root_Object.Object_Access);

   pragma inline (Get_R14_Super);
   pragma inline (Put_R14_Super);
   ...
   -- EVENT DECLARATION
   --
   type Events_sbD_Event_Type is new Events_Event_Type with
   record
   ...
   --    
   type Events_sbD_Instance_Event_Type is new Events_sbD_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type Events_sbD_Description_Event is new Events_sbD_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type Events_SprA1 is new Events_sbD_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;

   type Events_SprA2 is new Events_sbD_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;

   type Events_SprA3 is new Events_sbD_Instance_Event_Type with
   record 
      null;
   end record;

   type Events_SprA4 is new Events_sbD_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;

   type Events_SprA5 is new Events_sbD_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;

   type Events_SprA6 is new Events_sbD_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;
   ...
   package Events_sbD_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   Events_sbD_Queue: Events_sbD_Event_Queue.FIFO_Type;
