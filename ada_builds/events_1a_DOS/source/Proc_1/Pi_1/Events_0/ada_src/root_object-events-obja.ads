...
--* Description:             
--*                          Domain specification Root_Object.EventsobjA
...
--  ----------------------------------  
--  package Root_Object.Events.objA
...
package Root_Object.Events.objA is
--
------------------------------------------------------------------------
   --
   ...
   --
   -- Object objA participates in a super sub type relationship
   --
   --
   --
   ...
   --   Supertype_Object.
   --
   -- Object objA is a SUPER type of 
   --    objB,
   --    objC
   --    via Relationship R1
   --
   ...
   type Events_objA_Type is new Events_Type with record
      ...
      R1_Sub : Root_Object.Object_Access := null;
   ...
   procedure Put_R1_Sub (
      This_Object :   in  Root_Object.Object_Access;
      R1_Sub_Value : in Root_Object.Object_Access);

   pragma inline (Get_R1_Sub);
   pragma inline (Put_R1_Sub);
   ...
   -- EVENT DECLARATION
   --
   type Events_objA_Event_Type is new Events_Event_Type with
   record
   ...
   --    
   type Events_objA_Instance_Event_Type is new Events_objA_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type Events_objA_Description_Event is new Events_objA_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type Events_objA1 is new Events_objA_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
      Test_Number : Application_Types.Base_Integer_Type;
   end record;

   type Events_objA2 is new Events_objA_Instance_Event_Type with
   record 
      Test_Number : Application_Types.Base_Integer_Type;
   end record;
   ...
   package Events_objA_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   Events_objA_Queue: Events_objA_Event_Queue.FIFO_Type;
