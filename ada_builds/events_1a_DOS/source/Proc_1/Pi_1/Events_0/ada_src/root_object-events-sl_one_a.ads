...
--*************************************************************************************
--*                                                                                   *
--* File Name:               r.Events.SL_ONE_A.ads
...
--* Description:             
--*                          Domain specification Root_Object.EventsSL_ONE_A
...
--  ----------------------------------  
--  package Root_Object.Events.SL_ONE_A
...
package Root_Object.Events.SL_ONE_A is
--
------------------------------------------------------------------------
   --
   --**************  Object Sub_Level_1_A Information **************
   --
   -- Object SL_ONE_A is PASSIVE
   --
   -- Object SL_ONE_A participates in a super sub type relationship
   --
   --
   --
   -- Object SL_ONE_A is a SUPER type of 
   --    SL_TWO_A,
   --    SL_TWO_B
   --    via Relationship R10
   --
   -- Object SL_ONE_A is a SUB type of TL via Relationship R9
   --
   -- Object SL_ONE_A has an identifying non-referential attribute SL1A_id 
   -- and therefore may be uniquely created.
   -- 
   -- Object SL_ONE_A has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "SL_ONE_A";

   --
   -------------------------------------------------------------------------------------------------
   --

   type Events_SL_ONE_A_Type is new Events_Type with record

      --
      -- Non initialised identifying non referential
      SL1A_id : Application_Types.Base_Integer_Type;
      ...
      R10_Sub : Root_Object.Object_Access := null;
   ...
   function Get_SL1A_id (
      This_Object : Root_Object.Object_Access) 
   return Application_Types.Base_Integer_Type;

   procedure Put_SL1A_id (
      This_Object : in Root_Object.Object_Access;
      SL1A_id_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_SL1A_id);
   pragma inline (Put_SL1A_id);
   ...
   function  Get_R10_Sub (
      This_Object : Root_Object.Object_Access) 
   return Root_Object.Object_Access;

   procedure Put_R10_Sub (
      This_Object :   in  Root_Object.Object_Access;
      R10_Sub_Value : in Root_Object.Object_Access);

   pragma inline (Get_R10_Sub);
   pragma inline (Put_R10_Sub);
   ...
   --
   type Events_SL_ONE_A_Event_Type is new Events_Event_Type with
   record
      This_Event_Class : Event_Class;
   end record;

   procedure Event_Action (
      Dispatch_Event : in     Events_SL_ONE_A_Event_Type;
      This_Event     : in out Root_Object.Root_Event_Access_Type);


   --    
   type Events_SL_ONE_A_Instance_Event_Type is new Events_SL_ONE_A_Event_Type with
   record
   ...
   -- 
   procedure Event_Action (
      Dispatch_Event : in     Events_SL_ONE_A_Instance_Event_Type;
      This_Event     : in out Root_Object.Root_Event_Access_Type);



   ------------------------------------------------------------------------
   type Events_SL_ONE_A_Description_Event is new Events_SL_ONE_A_Instance_Event_Type with
   record
      null;
   end record;
   --
   ...
   --
   --
   type Events_TL1 is new Events_SL_ONE_A_Instance_Event_Type with
   record 
      null;
   end record;
   --
   type Events_TL2 is new Events_SL_ONE_A_Instance_Event_Type with
   record 
      Test_No: Application_Types.Base_Integer_Type;
   end record;

end Root_Object.Events.SL_ONE_A;
