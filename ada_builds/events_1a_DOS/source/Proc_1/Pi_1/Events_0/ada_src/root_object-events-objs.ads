...
--* Description:             
--*                          Domain specification Root_Object.EventsobjS
...
--  ----------------------------------  
--  package Root_Object.Events.objS
...
package Root_Object.Events.objS is
--
------------------------------------------------------------------------
   --
   --**************  Object Object_S Information **************
   --
   -- Object objS is ACTIVE
   --
   -- Object objS participates in a super sub type relationship
   --
   --
   --
   -- Object objS is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Idle,
   --   Subtype_State.
   --
   -- Object objS is a SUB type of objP via Relationship R4
   --
   -- Object objS does not have an identifying non-referential attribute and 
   -- therefore may not be uniquely created.
   -- 
   -- Object objS has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "objS";

   --
   -------------------------------------------------------------------------------------------------
   --

   type Events_objS_Type is new Events_Type with record
      ...
      --
      -- Initialised Non identifying non referential
      Result_S : Application_Types.Base_Integer_Type := Application_Types.Base_Integer_Type_First;
   ...
   procedure Put_Result_S (
      This_Object : in Root_Object.Object_Access;
      Result_S_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_Result_S);
   pragma inline (Put_Result_S);
   ...
   -- EVENT DECLARATION
   --
   type Events_objS_Event_Type is new Events_Event_Type with
   record
   ...
   --    
   type Events_objS_Instance_Event_Type is new Events_objS_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type Events_objS_Description_Event is new Events_objS_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type Events_objS1 is new Events_objS_Instance_Event_Type with
   record 
      null;
   end record;

   type Events_objP1 is new Events_objS_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;

   type Events_objP2 is new Events_objS_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
   end record;
   ...
   package Events_objS_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   Events_objS_Queue: Events_objS_Event_Queue.FIFO_Type;
