...
--* Description:             
--*                          Domain specification Root_Object.EventsSC
...
package Root_Object.Events.SC is
--
------------------------------------------------------------------------
   --
   --**************  Object State_Check Information **************
   --
   -- Object SC is ACTIVE
   --
   --
   --
   -- Object SC is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Idle,
   --   Check_State.
   --
   -- Object SC has an identifying non-referential attribute The_ID 
   -- and therefore may be uniquely created.
   -- 
   -- Object SC has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "SC";

   --
   -------------------------------------------------------------------------------------------------
   --

   type Events_SC_Type is new Events_Type with record

      --
      -- Non initialised identifying non referential
      The_ID : Application_Types.Base_Integer_Type;
      ...
      --
      -- Initialised Non identifying non referential
      Whats_The_State : Application_Types.Base_Text_Type := Application_Types.Base_Text_Type_First;
   ...
   procedure Put_The_ID (
      ...
      The_ID_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_The_ID);
   pragma inline (Put_The_ID);
   ...
   procedure Put_Whats_The_State (
      This_Object : in Root_Object.Object_Access;
      Whats_The_State_Value : in Application_Types.Base_Text_Type);

   pragma inline (Get_Whats_The_State);
   pragma inline (Put_Whats_The_State);
   ...
   -- EVENT DECLARATION
   --
   type Events_SC_Event_Type is new Events_Event_Type with
   record
   ...
   --    
   type Events_SC_Instance_Event_Type is new Events_SC_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type Events_SC_Description_Event is new Events_SC_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type Events_SC1 is new Events_SC_Instance_Event_Type with
   record 
      Test_No : Application_Types.Base_Integer_Type;
   end record;

   type Events_SC2 is new Events_SC_Instance_Event_Type with
   record 
      Test_No : Application_Types.Base_Integer_Type;
   end record;
   ...
   package Events_SC_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   Events_SC_Queue: Events_SC_Event_Queue.FIFO_Type;
