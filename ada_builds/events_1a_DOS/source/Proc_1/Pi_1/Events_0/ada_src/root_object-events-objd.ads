...
--* Description:             
--*                          Domain specification Root_Object.EventsobjD
...
--  ----------------------------------  
--  package Root_Object.Events.objD
...
package Root_Object.Events.objD is
--
------------------------------------------------------------------------
   --
   --**************  Object Object_D Information **************
   --
   -- Object objD is ACTIVE
   --
   -- Object objD participates in a super sub type relationship
   --
   --
   --
   -- Object objD is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Idle,
   --   Supertype_Object.
   --
   -- Object objD is a SUPER type of 
   --    objE,
   --    objF
   --    via Relationship R5
   --
   -- Object objD has an identifying non-referential attribute ReferenceD 
   -- and therefore may be uniquely created.
   -- 
   -- Object objD has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "objD";

   --
   -------------------------------------------------------------------------------------------------
   --

   type Events_objD_Type is new Events_Type with record
      ...
      --
      -- Initialised Non identifying non referential
      no_of_received_events : Application_Types.Base_Integer_Type := Application_Types.Base_Integer_Type_First;
      ...
      R5_Sub : Root_Object.Object_Access := null;
   ...
   procedure Put_no_of_received_events (
      This_Object : in Root_Object.Object_Access;
      no_of_received_events_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_no_of_received_events);
   pragma inline (Put_no_of_received_events);
   ...
   procedure Put_R5_Sub (
      This_Object :   in  Root_Object.Object_Access;
      R5_Sub_Value : in Root_Object.Object_Access);

   pragma inline (Get_R5_Sub);
   pragma inline (Put_R5_Sub);
   ...
   -- EVENT DECLARATION
   --
   type Events_objD_Event_Type is new Events_Event_Type with
   record
   ...
   --    
   type Events_objD_Instance_Event_Type is new Events_objD_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type Events_objD_Description_Event is new Events_objD_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type Events_objD1 is new Events_objD_Instance_Event_Type with
   record 
      Test_Number : Application_Types.Base_Integer_Type;
   end record;

   type Events_objD2 is new Events_objD_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
      Test_Number : Application_Types.Base_Integer_Type;
   end record;

   type Events_objD3 is new Events_objD_Instance_Event_Type with
   record 
      Test : Application_Types.Base_Integer_Type;
      Test_Number : Application_Types.Base_Integer_Type;
   end record;

   type Events_objD4 is new Events_objD_Instance_Event_Type with
   record 
      null;
   end record;
   ...
   package Events_objD_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   Events_objD_Queue: Events_objD_Event_Queue.FIFO_Type;
