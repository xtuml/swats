...
--* Description:             
--*                          Domain specification Root_Object.EventsobjZ
...
--  ----------------------------------  
--  package Root_Object.Events.objZ
...
package Root_Object.Events.objZ is
--
------------------------------------------------------------------------
   --
   --**************  Object Object_Z Information **************
   --
   -- Object objZ is ACTIVE
   --
   --
   --
   -- Object objZ is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   Idle,
   --   Add,
   --   Check.
   --
   -- Object objZ has an identifying non-referential attribute objZ_id 
   -- and therefore may be uniquely created.
   -- 
   -- Object objZ has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "objZ";

   --
   -------------------------------------------------------------------------------------------------
   --

   type Events_objZ_Type is new Events_Type with record

      --
      -- Non initialised identifying non referential
      objZ_id : Application_Types.Base_Integer_Type;
      ...
      --
      -- Initialised Non identifying non referential
      Int_Att : Application_Types.Base_Integer_Type := Application_Types.Base_Integer_Type_First;
   ...
   procedure Put_objZ_id (
      ...
      objZ_id_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_objZ_id);
   pragma inline (Put_objZ_id);
   ...
   procedure Put_Int_Att (
      This_Object : in Root_Object.Object_Access;
      Int_Att_Value : in Application_Types.Base_Integer_Type);

   pragma inline (Get_Int_Att);
   pragma inline (Put_Int_Att);
   ...
   -- EVENT DECLARATION
   --
   type Events_objZ_Event_Type is new Events_Event_Type with
   record
   ...
   --    
   type Events_objZ_Instance_Event_Type is new Events_objZ_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type Events_objZ_Description_Event is new Events_objZ_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type Events_objZ1 is new Events_objZ_Instance_Event_Type with
   record 
      null;
   end record;

   type Events_objZ2 is new Events_objZ_Instance_Event_Type with
   record 
      null;
   end record;

   type Events_objZ3 is new Events_objZ_Instance_Event_Type with
   record 
      null;
   end record;

   type Events_objZ4 is new Events_objZ_Instance_Event_Type with
   record 
      null;
   end record;
   ...
   package Events_objZ_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   Events_objZ_Queue: Events_objZ_Event_Queue.FIFO_Type;
