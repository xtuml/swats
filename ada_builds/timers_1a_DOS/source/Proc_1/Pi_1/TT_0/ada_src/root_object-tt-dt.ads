...
--* Description:             
--*                          Domain specification Root_Object.TTDT
...
package Root_Object.TT.DT is
--
------------------------------------------------------------------------
   --
   --**************  Object Deleting_Timer Information **************
   --
   -- Object DT is ACTIVE
   --
   -- Object DT participates in a super sub type relationship
   --
   --
   --
   -- Object DT is ACTIVE and has the following state actions defined
   --   Non_Existent,
   --   idle,
   --   executing_part_one,
   --   failed_part_one,
   ...
   --   deletion.
   --
   -- Object DT is a SUB type of T via Relationship R2
   --
   -- Object DT has an identifying non-referential attribute id 
   -- and therefore may be uniquely created.
   -- 
   -- Object DT has no TAGS.
   --
   ------------------------------------------------------------------------
   -- *********************  State Actions ********************************
   ------------------------------------------------------------------------
   ...
   Object_String : String := "DT";

   --
   -------------------------------------------------------------------------------------------------
   --

   type TT_DT_Type is new TT_Type with record
   ...
   -- EVENT DECLARATION
   --
   type TT_DT_Event_Type is new TT_Event_Type with
   record
   ...
   --    
   type TT_DT_Instance_Event_Type is new TT_DT_Event_Type with
   record
   ...
   ------------------------------------------------------------------------
   type TT_DT_Description_Event is new TT_DT_Instance_Event_Type with
   record
      null;
   end record;  
   --
   ...
   type TT_DT1 is new TT_DT_Instance_Event_Type with
   record 
      null;
   end record;

   type TT_DT2 is new TT_DT_Instance_Event_Type with
   record 
      null;
   end record;

   type TT_DT3 is new TT_DT_Instance_Event_Type with
   record 
      null;
   end record;

   type TT_DT4 is new TT_DT_Instance_Event_Type with
   record 
      null;
   end record;

   type TT_DT5 is new TT_DT_Instance_Event_Type with
   record 
      null;
   end record;

   type TT_DT6 is new TT_DT_Instance_Event_Type with
   record 
      null;
   end record;

   type TT_T1 is new TT_DT_Instance_Event_Type with
   record 
      null;
   end record;

   type TT_T2 is new TT_DT_Instance_Event_Type with
   record 
      null;
   end record;
   ...
   package TT_DT_Event_Queue is new Generic_FIFO (Root_Object.Root_Event_Access_Type); 

   TT_DT_Queue: TT_DT_Event_Queue.FIFO_Type;
